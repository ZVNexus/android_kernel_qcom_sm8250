...
extern int32_t adc_tm5_channel_measure(struct adc_tm_chip *chip,
...
int hid_to_set_station_cover_state(u8 type) // 1:Close , 0:Open
{
	...
	if (type && g_station_sleep && g_station_dp_disconnect &&
	    !ec_i2c_ultra_mode) {
...
int ec_i2c_check_interrupt(char *type, char *event)
{
	...
	if ((gDongleType == 100 || gDongleType == 200 || gDongleType == 2) &&
	    (ec_i2c_is_suspend == 1)) {
				   ...
				   const char *data, size_t count)
{
	...
	if (val == 0) {
		...
		if (1 == is_porta_cc_locked && state == 1) {
			...
			blocking_notifier_call_chain(&ec_hid_event_header,
						     gDongleType, NULL);
			ec_hid_uevent();
...
void ec_hid_uevent(void)
{
	...
	if (type == Dongle_Station2 && station_shutdown) {
		...
		return;
	} else if (type == Dongle_Station2 &&
		   g_hid_data->previous_event ==
...
void update_POGO_ID_ADC_Threshold(enum asus_dongle_type type)
{
	...
	case Dongle_NO_INSERT:
		...
		msleep(20);
		rc = adc_tm5_channel_measure(adc_tm_dev_pogo,
					     &adc_tm_param_pogo);
		if (rc)
...
static ssize_t lock_show(struct device *dev, struct device_attribute *mattr,
			 char *buf)
{
...
static ssize_t lock_store(struct device *dev, struct device_attribute *mattr,
			  const char *data, size_t count)
{
	int ret = 0;
	u32 val;
...
static int start_ec_adc_measure(void)
{
	...
	if (adc_tm_dev_pogo == NULL) {
		...
		adc_tm_param_pogo.threshold_notification =
			qpnp_pogo_id_adc_notification;
		msleep(20);
		rc = adc_tm5_channel_measure(adc_tm_dev_pogo,
					     &adc_tm_param_pogo);
		if (rc < 0)
				  ...
				  const char *data, size_t count)
{
	...
	if (val == 1) {
		...
		else if (drm_panel_notifier_register(
				 active_panel, &g_hid_data->display_notifier) <
			 0)
		...
		if (active_panel) {
			if (drm_panel_notifier_unregister(
				    active_panel,
				    &g_hid_data->display_notifier) < 0)
...
static DEVICE_ATTR(gDongleType, S_IRUGO | S_IWUSR, gDongleType_show,
		   gDongleType_store);
static DEVICE_ATTR(lock, S_IRUGO | S_IWUSR, lock_show, lock_store);
static DEVICE_ATTR(pogo_mutex, S_IRUGO | S_IWUSR, pogo_mutex_show,
		   pogo_mutex_store);
static DEVICE_ATTR(block_input, S_IRUGO | S_IWUSR, block_input_show,
		   block_input_store);
static DEVICE_ATTR(start_ec_adc_tm5, S_IRUGO | S_IWUSR,
		   start_ec_adc_measure_show, start_ec_adc_measure_store);
static DEVICE_ATTR(sync_state, S_IRUGO | S_IWUSR, NULL, sync_state_store);
static DEVICE_ATTR(gDongleEvent, S_IRUGO | S_IWUSR, gDongleEvent_show,
		   gDongleEvent_store);
static DEVICE_ATTR(pogo_sleep, S_IRUGO | S_IWUSR, pogo_sleep_show,
		   pogo_sleep_store);
static DEVICE_ATTR(pogo_detect, S_IRUGO | S_IWUSR, pogo_detect_show,
		   pogo_detect_store);
static DEVICE_ATTR(pogo_fan_on, S_IRUGO | S_IWUSR, pogo_fan_on_show,
		   pogo_fan_on_store);
static DEVICE_ATTR(pogo_aura_en, S_IRUGO | S_IWUSR, pogo_aura_en_show,
		   pogo_aura_en_store);
static DEVICE_ATTR(pogo_temp_intr, S_IRUGO | S_IWUSR, pogo_temp_intr_show,
		   pogo_temp_intr_store);
static DEVICE_ATTR(pogo_id_voltage, S_IRUGO | S_IWUSR, pogo_id_voltage_show,
		   NULL);
static DEVICE_ATTR(pogo_sync_key, S_IRUGO | S_IWUSR, pogo_sync_key_show,
		   pogo_sync_key_store);
static DEVICE_ATTR(is_ec_has_removed, S_IRUGO | S_IWUSR, is_ec_has_removed_show,
		   is_ec_has_removed_store);
static DEVICE_ATTR(inbox_audio_switch, S_IRUGO | S_IWUSR,
		   inbox_audio_switch_show, inbox_audio_switch_store);
static DEVICE_ATTR(drm_panel, S_IRUGO | S_IWUSR, NULL, drm_panel_register);
static DEVICE_ATTR(touch_recovery, S_IRUGO | S_IWUSR, touch_recovery_status,
		   touch_recovery_store);

static struct attribute *ec_hid_attrs[] = { &dev_attr_gDongleType.attr,
					    &dev_attr_lock.attr,
...
void do_ec_porta_cc_connect(void)
{
	...
	if (1 == is_porta_cc_locked && state == 1) {
		...
		ec_set_dp_display.i2c_set_display_bl(
			get_last_backlight_value());
		/* ASUS BSP DP --- */

		blocking_notifier_call_chain(&ec_hid_event_header, gDongleType,
					     NULL);
		ec_hid_uevent();
		is_porta_cc_locked = 1;
	} else if (1 == is_porta_cc_locked && state == 0) {
...
int ec_hid_display_notifier_call(struct notifier_block *self,
				 unsigned long event, void *data)
{
	struct ec_hid_data *p_data =
		container_of(self, struct ec_hid_data, display_notifier);
	...
	if (evdata->data && event == DRM_PANEL_EVENT_BLANK && p_data) {
		...
		case DRM_PANEL_BLANK_POWERDOWN:
			...
			printk("[EC_HID] Panel Off: Power Saving %d, DP Disconnect %d, Lid status %d\n",
			       g_station_sleep, g_station_dp_disconnect,
			       lid_status);
			hid_switch_usb_autosuspend(true);
			//block_hid_input = true;
			hid_to_set_phone_panel_state(0);

			if (g_station_sleep && g_station_dp_disconnect &&
			    lid_status && !ec_i2c_ultra_mode) {
				hid_to_set_ultra_power_mode(1);
			}
...
static int ec_hid_parse_dt(struct device *dev,
			   struct ec_hid_data *ec_hid_device)
{
	...
	ec_hid_device->pogo_sleep =
		of_get_named_gpio_flags(np, "dongle,pogo-sleep", 0, NULL);
	printk("[EC_HID] pogo_sleep : %d\n", ec_hid_device->pogo_sleep);

	ec_hid_device->pogo_det =
		of_get_named_gpio_flags(np, "dongle,pogo-det", 0, NULL);
...
*/
	...
	ec_hid_device->pins_default =
		pinctrl_lookup_state(ec_hid_device->pinctrl, "default");
	...
	retval = pinctrl_select_state(ec_hid_device->pinctrl,
				      ec_hid_device->pins_default);
	if (retval)
...
static int ec_hid_probe(struct platform_device *pdev)
{
	...
	dev = device_create(ec_hid_class, &pdev->dev, ec_hid_device->devt,
			    ec_hid_device, "dongle");
	...
	drm_registered = false;
	ec_hid_device->display_notifier.notifier_call =
		ec_hid_display_notifier_call;
	...
	asus_pogo_vadc_chan =
		iio_channel_get(g_hid_data->dev, "asus_pogo_vadc");
