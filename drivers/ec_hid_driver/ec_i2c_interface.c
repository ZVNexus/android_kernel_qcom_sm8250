...
int ec_i2c_write(struct i2c_client *client, char *writebuf, int writelen)
{
	...
	if (writelen > 0) {
		...
		for (i = 0; i < I2C_RETRY_NUMBER; i++) {
			...
			if (ret < 0) {
				printk("[EC_I2C] : %s:i2c_transfer(write) error, ret=%d",
				       __func__, ret);
			} else
				break;
		}
...
* Return: fail <0
***********************************************************************/
int ec_i2c_read(struct i2c_client *client, char *writebuf, int writelen,
...
static ssize_t fps_show(struct device *dev, struct device_attribute *mattr,
			char *buf)
{
	int ret = 0;
	char buffer[2] = { 0 };
...
static int i2c_get_station_hwid(void)
{
	...
	if ((buffer[1] == 1) && (buffer[2] == 1))
		...
		if (strncmp(&model_name[1], "ZS660KLS", 8) == 0) {
			printk("[EC_I2C] model_name: ZS660KLS, force set HWID is ROG_Station2",
			       model_name);
			Station_HWID = ROG_Station2;
		} else
			Station_HWID = ROG_Station3;
...
static ssize_t hw_id_show(struct device *dev, struct device_attribute *mattr,
			  char *buf)
{
...
static ssize_t ec_ssn_show(struct device *dev, struct device_attribute *mattr,
			   char *buf)
{
	int ret = 0;
	char buffer[23] = { 0 };
...
static ssize_t ec_ssn_store(struct device *dev, struct device_attribute *mattr,
			    const char *data, size_t count)
{
	int ret = 0;
	char cmd[13] = { 0 };
...
static ssize_t uart_show(struct device *dev, struct device_attribute *mattr,
			 char *buf)
{
	char buffer = 0;
	int ret = 0;
...
static ssize_t uart_store(struct device *dev, struct device_attribute *mattr,
			  const char *data, size_t count)
{
	int ret = 0;
	u32 val;
...
static ssize_t sd_power_show(struct device *dev, struct device_attribute *mattr,
			     char *buf)
{
	/*
	char buffer =0;
...
static ssize_t duty_store(struct device *dev, struct device_attribute *mattr,
			  const char *data, size_t count)
{
	int duty = 0;
	char cmd[3] = { 0 };
...
static ssize_t freq_store(struct device *dev, struct device_attribute *mattr,
			  const char *data, size_t count)
{
	int freq = 0;
	char cmd[3] = { 0 };
...
static ssize_t rpm_show(struct device *dev, struct device_attribute *mattr,
			char *buf)
{
	int ret = 0;
	int RPM = 0;
...
static ssize_t pwm_store(struct device *dev, struct device_attribute *mattr,
			 const char *data, size_t count)
{
	int ret = 0;
	char cmd[2] = { 0 };
...
static ssize_t fw_ver_show(struct device *dev, struct device_attribute *mattr,
			   char *buf)
{
	int ret = 0;
...
static ssize_t get_gpio_show(struct device *dev, struct device_attribute *mattr,
			     char *buf)
{
	int ret = 0;
	char buffer = 0;
...
static ssize_t u0504_show(struct device *dev, struct device_attribute *mattr,
			  char *buf)
{
	int ret = 0;
	int state;
...
static DEVICE_ATTR(factory_mode, S_IRUGO | S_IWUSR, factory_mode_show,
		   factory_mode_store);
static DEVICE_ATTR(StationPDO_voltage_max, S_IRUGO | S_IWUSR, PDO_voltage_show,
		   NULL);
static DEVICE_ATTR(StationPDO_current_max, S_IRUGO | S_IWUSR, PDO_current_show,
		   NULL);
...
static DEVICE_ATTR(init_state, S_IRUGO | S_IWUSR, init_state_show,
		   init_state_store);
static DEVICE_ATTR(pwm_freq_duty, S_IRUGO | S_IWUSR, pwm_freq_duty_show, NULL);
static DEVICE_ATTR(battery_48_hours_state, S_IRUGO | S_IWUSR,
		   battery_48_hours_state_show, battery_48_hours_state_store);
#ifdef ASUS_FACTORY_BUILD
static DEVICE_ATTR(ec_ssn, S_IRUGO | S_IWUSR, ec_ssn_show, ec_ssn_store);
static DEVICE_ATTR(model_name, S_IRUGO | S_IWUSR, model_name_show,
		   model_name_store);
#else
...
static DEVICE_ATTR(disable_charger_suspend, S_IRUGO | S_IWUSR, NULL,
		   disable_charger_suspend_store);
static DEVICE_ATTR(set_cover_state, S_IRUGO | S_IWUSR, NULL,
		   set_cover_state_store);
static DEVICE_ATTR(latch_key_state, S_IRUGO | S_IWUSR, latch_key_state_show,
		   NULL);
...
static DEVICE_ATTR(disconnect_porta_cc, S_IRUGO | S_IWUSR, NULL,
		   disconnect_porta_cc_store);
static DEVICE_ATTR(connect_porta_cc, S_IRUGO | S_IWUSR, NULL,
		   connect_porta_cc_store);
static DEVICE_ATTR(set_color_temp, S_IRUGO | S_IWUSR, NULL,
		   set_color_temp_store);
static DEVICE_ATTR(restore_display_config, S_IRUGO | S_IWUSR, NULL,
		   restore_display_config_store);
...
static DEVICE_ATTR(APL6001_AdapterID, S_IRUGO | S_IWUSR, apl6001_adc2_show,
		   NULL);
...
int ec_i2c_create_sysfs(struct i2c_client *client)
{
	...
	device_create(ec_i2c_class, &client->dev, ec_i2c_data->devt,
		      ec_i2c_data, "dongle");
...
static int ec_i2c_probe(struct i2c_client *client,
			const struct i2c_device_id *id)
{
	...
	platform_data = devm_kzalloc(
		&client->dev, sizeof(struct ec_i2c_platform_data), GFP_KERNEL);
	if (!platform_data) {
	...
	ec_battery_func.i2c_set_ultra_low_power_mode =
		i2c_set_ultra_low_power_mode;
	ec_battery_func.i2c_set_phone_panel_state = i2c_set_phone_panel_state;
	ec_battery_func.i2c_set_station_cover_state =
		i2c_set_station_cover_state;
