...
static struct timeval srcdisconnect_gettime;
static unsigned long srcdisconnect_record = 0;
static unsigned long srcdisconnect_interval = 0;
static int srcdisconnect_retry = 2;
...
#ifdef CONFIG_TYPEC_CAP_A2C_C2C
DECL_TCPC_TIMEOUT(TYPEC_RT_TIMER_A2C_C2C, 5),
#endif	/* CONFIG_TYPEC_CAP_A2C_C2C */
#ifdef CONFIG_USB_POWER_DELIVERY
...
void tcpc_enable_timer(struct tcpc_device *tcpc, uint32_t timer_id)
{
	...
	//printk("tcpc_enable_timer, tcpc_timer_name = %s\n", tcpc_timer_name[timer_id]);

if(timer_id == TYPEC_TIMER_SRCDISCONNECT){
	//printk("tcpc_enable_timer, TYPEC_TIMER_SRCDISCONNECT enter\n");

	do_gettimeofday(&srcdisconnect_gettime);

	srcdisconnect_interval = srcdisconnect_gettime.tv_sec - srcdisconnect_record;

	//printk("tcpc_enable_timer, TYPEC_TIMER_SRCDISCONNECT enter, srcdisconnect_gettime.tv_sec = %lu\n", srcdisconnect_gettime.tv_sec);
	//printk("tcpc_enable_timer, TYPEC_TIMER_SRCDISCONNECT enter, srcdisconnect_record = %lu\n", srcdisconnect_record);
	//printk("tcpc_enable_timer, TYPEC_TIMER_SRCDISCONNECT enter, do_gettimeofday_diff = %lu\n", srcdisconnect_interval);
	printk("tcpc_enable_timer, TYPEC_TIMER_SRCDISCONNECT enter, srcdisconnect_retry = %d\n", srcdisconnect_retry);
	if(srcdisconnect_retry == 0){
		tout = 80000;
		srcdisconnect_retry = 2;
		printk("tcpc_enable_timer, TYPEC_TIMER_SRCDISCONNECT enter, change src disconnect time\n");
	} else if(srcdisconnect_interval <= 1){
		srcdisconnect_retry = srcdisconnect_retry -1;
	}
	else{
		srcdisconnect_retry = 2;
	}
	srcdisconnect_record = srcdisconnect_gettime.tv_sec;
	printk("tcpc_enable_timer, srcdisconnect_record = %lu\n", srcdisconnect_record);
}
