...
static inline int rt1711h_check_revision(struct i2c_client *client)
{
	...
	ret = i2c_smbus_read_i2c_block_data(client, TCPC_V10_REG_VID, 2, (u8 *)&vid);
	if (ret < 0) {
	...
	ret = i2c_smbus_read_i2c_block_data(client, TCPC_V10_REG_PID, 2, (u8 *)&pid);
	if (ret < 0) {
	...
	ret = i2c_smbus_write_byte_data(client, RT1711H_REG_SWRESET, data);
	if (ret < 0)
		return ret;

	usleep_range(1000, 2000);

	ret = i2c_smbus_read_i2c_block_data(client, TCPC_V10_REG_DID, 2, (u8 *)&did);
	if (ret < 0) {
				...
				const struct i2c_device_id *id)
{
	...
	chip->irq_wake_lock = wakeup_source_register(NULL,
