...
static struct attribute *lid_attrs[] = { &dev_attr_lid_status.attr, NULL };
...
extern int hid_to_set_station_cover_state(
	u8 type); // ASUS_BSP Deeo : add for ACCY EC driver
static void lid_report_function(struct work_struct *work)
{
					 ...
					 bool active)
{
	...
	if (active) {
		set_state =
			pinctrl_lookup_state(key_pinctrl, "asustek_lid_active");
		if (IS_ERR(set_state)) {
