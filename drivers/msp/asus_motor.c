...
static ssize_t motor_atd_status_proc_write(struct file *filp,
...
static ssize_t motor_solo_power_write(struct file *filp,
				      const char __user *buff, size_t len,
				      loff_t *data)
{
	ssize_t ret_len;
...
static ssize_t motor_manual_proc_write(struct file *filp,
				       const char __user *buff, size_t len,
				       loff_t *data)
{
	ssize_t ret_len;
	...
	if (rc < 0 || g_motor_degree < 0 || g_motor_degree > 180) {
		...
		rc = MSP430FR2311_Set_ManualMode(
			g_motor_dir, g_motor_degree,
			g_motor_speed); //speed set to 5 by default
...
static ssize_t motor_auto_proc_write(struct file *filp, const char __user *buff,
...
static ssize_t motor_stop_proc_write(struct file *filp, const char __user *buff,
				     size_t len, loff_t *data)
{
	ssize_t ret_len;
	char messages[16] = "";
...
static ssize_t motor_param_proc_write(struct file *filp,
				      const char __user *buff, size_t len,
				      loff_t *data)
{
#define MAX_PARAM_MSG_LEN 100
	...
	sscanf(messages, "%d %d %d %d %d %d %d %d %d %d %d %d %d %d", &val[0],
	...
	if (val[13] != 0xfe) {
		...
		if (1) { //(((val[13]&0x0F) >= 0) && ((val[13]&0x0F) <= 10))
			sscanf(messages,
			       ...
			       &Zen7Val[0], &Zen7Val[1], &Zen7Val[2],
			       &Zen7Val[3], &Zen7Val[4], &Zen7Val[5],
			       &Zen7Val[6], &Zen7Val[7], &Zen7Val[8],
			       &Zen7Val[9], &Zen7Val[10], &Zen7Val[11],
			       &Zen7Val[12], &Zen7Val[13], &Zen7Val[14],
			       &Zen7Val[15], &Zen7Val[16], &Zen7Val[17],
			       &Zen7Val[18], &Zen7Val[19]);

			pr_err("[MCU] long cmd = %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",
			       Zen7Val[0], Zen7Val[1], Zen7Val[2], Zen7Val[3],
			       Zen7Val[4], Zen7Val[5], Zen7Val[6], Zen7Val[7],
			       Zen7Val[8], Zen7Val[9], Zen7Val[10], Zen7Val[11],
			       Zen7Val[12], Zen7Val[13], Zen7Val[14],
			       Zen7Val[15], Zen7Val[16], Zen7Val[17],
			       Zen7Val[18], Zen7Val[19]);
		...
		rc = MSP430FR2311_Set_ParamMode(
			val); //speed set to 5 by default
	}
...
static ssize_t motor_angle_proc_write(struct file *filp,
				      const char __user *buff, size_t len,
				      loff_t *data)
{
	ssize_t ret_len;
...
static int motor_drv_proc_read(struct seq_file *buf, void *v)
{
	...
	if (rc == 0) {
		seq_printf(
			buf,
			"[MCU] drv state:%02X %02X %02X %02X %02X %02X %02X\n",
			drv_state[0], drv_state[1], drv_state[2], drv_state[3],
			drv_state[4], drv_state[5], drv_state[6]);
	} else {
...
static ssize_t motor_drv_proc_write(struct file *filp, const char __user *buff,
				    size_t len, loff_t *data)
{
	ssize_t ret_len;
	char messages[16] = "";
	...
	sscanf(messages, "%d %d %d %d %d", &val[0], &val[1], &val[2], &val[3],
	       &val[4]);
...
static int motor_akm_proc_read(struct seq_file *buf, void *v)
{
	...
	if (rc == 0) {
		seq_printf(
			buf,
			"[MCU] akm raw data(reg:0x17):%02X %02X %02X %02X %02X %02X %02X %02X\n",
			akm_temp[0], akm_temp[1], akm_temp[2], akm_temp[3],
			akm_temp[4], akm_temp[5], akm_temp[6], akm_temp[7]);
	} else {
...
static ssize_t motor_akm_proc_write(struct file *filp, const char __user *buff,
				    size_t len, loff_t *data)
{
#define MAX_PARAM_MSG_LEN 100
	char messages[MAX_PARAM_MSG_LEN] = "";
...
static void create_proc_file(const char *PATH,
...
static void create_motor_proc_files_factory(void)
{
	...
	if (!has_created) {
		...
		create_proc_file(PROC_MOTOR_MANUAL_MODE,
				 &motor_manual_mode_fops);
		...
		create_proc_file(PROC_MOTOR_PROBE_STATUS,
				 &motor_probe_status_fops);
