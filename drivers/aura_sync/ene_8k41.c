...
static int ene_UpdateFirmware(struct i2c_client *client, char *fw_buf)
{
	...
	for (addr = 0x0000; addr <= (0x4000 - FLASH_BUF_LEN);
	     addr = addr + FLASH_BUF_LEN) {
		//printk("[AURA_SYNC] set addr : 0x%04x\n", addr);
		err = ene_8k41_write_bytes(client, 0xF808,
					   (addr & 0xFF)); //	Addr_L
		if (err != 1)
		...
		buf[0] = 0x05;
		memcpy((buf + 1), fw_buf + addr,
		       FLASH_BUF_LEN * sizeof(char)); // copy fw_buf to buf

		err = i2c_write_bytes(
			client, buf,
			(FLASH_BUF_LEN + 1) *
				sizeof(char)); // 128 byte + 1 cmd = 129
		if (err != 1)
			printk("[AURA_SYNC] i2c_write_bytes:err %d\n", err);
...
void bumper_switch(u32 val)
{
	...
	if (val > 0) {
		...
		if (!(data[0] & 0x01)) {
			...
			err = ene_8k41_write_bytes(g_pdata->client, 0xF203,
						   tmp);
			if (err != 1)
		...
		if (!(data[0] & 0x01)) {
			tmp = (data[0] & 0x21) | 0x01;
			err = ene_8k41_write_bytes(g_pdata->client, 0xF208,
						   tmp);
			if (err != 1)
				printk("[AURA_SYNC] ene_8k41_write_bytes:err %d\n",
				       err);
...
static ssize_t vdd2_store(struct device *dev, struct device_attribute *attr,
			  const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev->parent);
	struct ene_8k41_platform_data *platform_data =
...
static ssize_t vdd2_show(struct device *dev, struct device_attribute *attr,
			 char *buf)
{
	struct i2c_client *client = to_i2c_client(dev->parent);
	struct ene_8k41_platform_data *platform_data =
			 ...
			 const char *buf, size_t count)
{
	...
	if (val > 0) {
		...
		if (ret < 0) {
			printk("[AURA_SYNC] fail to enable ene8k41_regulator %d ",
			       ret);
			return count;
		}
	} else {
		...
		if (ret < 0) {
			printk("[AURA_SYNC] fail to disable ene8k41_regulator %d ",
			       ret);
			return count;
		}
	}
...
static ssize_t blink_set_store(struct device *dev,
			       struct device_attribute *attr, const char *buf,
			       size_t count)
{
	struct i2c_client *client = to_i2c_client(dev->parent);
...
static ssize_t blink_set_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	struct i2c_client *client = to_i2c_client(dev->parent);
	struct ene_8k41_platform_data *platform_data =
	...
	printk("[AURA_SYNC] ON:%d ms, OFF:%d ms\n", platform_data->on_ms,
	       platform_data->off_ms);
	return snprintf(buf, PAGE_SIZE, "ON:%d ms, OFF:%d ms\n",
			platform_data->on_ms, platform_data->off_ms);
}

static ssize_t emulate_store(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev->parent);
	struct ene_8k41_platform_data *platform_data =
...
static ssize_t emulate_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	struct i2c_client *client = to_i2c_client(dev->parent);
	struct ene_8k41_platform_data *platform_data =
...
static ssize_t suspend_vdd_store(struct device *dev,
				 struct device_attribute *attr, const char *buf,
				 size_t count)
{
	ssize_t ret;
...
static ssize_t suspend_vdd_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "suspend_vdd = %d\n", suspend_vdd_on);
}

static ssize_t pb_output_store(struct device *dev,
			       struct device_attribute *attr, const char *buf,
			       size_t count)
{
	struct i2c_client *client = to_i2c_client(dev->parent);
...
static ssize_t pb_output_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	struct i2c_client *client = to_i2c_client(dev->parent);
	unsigned char data[4] = { 0 };
...
static ssize_t pb_hl_store(struct device *dev, struct device_attribute *attr,
			   const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev->parent);
	u32 val;
...
static ssize_t pb_hl_show(struct device *dev, struct device_attribute *attr,
			  char *buf)
{
	struct i2c_client *client = to_i2c_client(dev->parent);
	unsigned char data[4] = { 0 };
...
static ssize_t CSCmode_store(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t count)
{
	ssize_t ret;
	u32 val;
...
static ssize_t CSCmode_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
			     ...
			     struct ene_8k41_platform_data *pdata)
{
	...
	pdata->aura_front_en = of_get_named_gpio_flags(
		np, "ene,aura_front_en", 0, &pdata->aura_front_en_flags);
	printk("[AURA_SYNC] aura_front_en : %d:%s\n", pdata->aura_front_en,
	       (pdata->aura_front_en == -ENOENT ? "NOT DEFINED!!" :
						  " DEFINED."));

	pdata->logo_5p0_en = of_get_named_gpio_flags(np, "ene,logo_5p0_en", 0,
						     &pdata->logo_5p0_en_flags);
			  ...
			  const struct i2c_device_id *id)
{
	...
	if (platform_data->aura_front_en != -ENOENT) {
		err = gpio_request(platform_data->aura_front_en,
				   "aura_front_en");
		if (err)
	...
	*/
	...
	platform_data->check_fw_workqueue =
		alloc_ordered_workqueue("check_fw_workqueue", 0);
	...
	queue_work(platform_data->check_fw_workqueue,
		   &platform_data->check_fw_work);
...
*/
	...
	platform_data->resume_workqueue =
		alloc_ordered_workqueue("resume_aura_workqueue", 0);
	...
	// Init wake lock
	wake_lock_init(&platform_data->aura_wake_lock, WAKE_LOCK_SUSPEND,
		       "aura_wake_lock");
