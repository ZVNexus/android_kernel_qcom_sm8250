...
static ssize_t get_value(struct device *dev, struct device_attribute *attr,
			 char *buf)
{
	struct i2c_client *client = to_i2c_client(dev->parent);
	struct sky81298_platform_data *platform_data =
...
void bumper_vdd_switch(u32 val)
{
	...
	if (val) {
		...
		err = sky81298_write_bytes(
			g_client, 0x8, 0x15); // set MM_EN1 & MM_EN2 & MM_EN3
		if (err != 1)
			printk("[Bumper] sky81298_write_bytes:err %d\n", err);
	} else {
		// Set Defautl register value
...
static struct attribute *pwm_attrs[] = { &dev_attr_register.attr,
					 &dev_attr_value.attr,
					 &dev_attr_VDD.attr, NULL };
...
static int bumper_led_register(struct device *dev,
			       struct sky81298_platform_data *pdata)
{
...
static int sky81298_probe(struct i2c_client *client,
			  const struct i2c_device_id *id)
{
	...
	platform_data =
		devm_kzalloc(&client->dev,
			     sizeof(struct sky81298_platform_data), GFP_KERNEL);
	if (!platform_data) {
