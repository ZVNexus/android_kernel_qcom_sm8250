...
static int ene_6k582_read_bytes(struct i2c_client *client, short addr,
				char *data)
{
	int err = 0;
	...
	pr_debug(
		"[AURA_STATION] ene_6k582_read_bytes : addr: 0x%x, buf[1] : 0x%x, buf[2] : 0x%x\n",
...
static int ene_6k582_write_bytes(struct i2c_client *client, short addr,
				 char value)
{
	int err = 0;
	unsigned char buf[16] = { 0 };
	...
	pr_debug(
		"[AURA_STATION] ene_6k582_write_bytes : addr: 0x%x, buf[1] : 0x%x, buf[2] : 0x%x, value : 0x%x\n",
...
static int ene_UpdateFirmware(struct i2c_client *client, char *fw_buf)
{
	...
	*/
	...
	for (addr = 0x0000; addr <= 0x6F80; addr += 0x80) {
		err = ene_6k582_write_bytes(client, 0xF808,
					    (addr & 0xFF)); //	Addr_L
		if (err != 1) {
		...
		err = ene_6k582_write_bytes(client, 0xF809,
					    (addr >> 8) & 0xFF); //	Addr_H
		if (err != 1) {
		...
		err = ene_6k582_write_bytes(client, 0xF807,
					    0x20); // erase page cmd
		if (err != 1)
			printk("[AURA_STATION] REG[0xF807]: write err %d\n",
			       err);
	...
	if (g_panel_state == 0) {
		...
		if (data_r[0] == 0xAA) {
			...
			if (data_r[0] == 0xAA) {
				...
				msleep(100);
				mutex_lock(&g_pdata->ene_mutex);
				err = ene_6k582_read_bytes(client, 0x8453,
							   data_r);
				if (err != 2) {
					printk("[AURA_STATION] sleep_show:err %d\n",
					       err);
				...
				sleep_val = 0xAA;
				mutex_lock(&g_pdata->ene_mutex);
				err = ene_6k582_write_bytes(client, 0x8453,
							    sleep_val);
				if (err != 1) {
					printk("[AURA_STATION] ene_6k582_write_bytes 0X8453-0xAA :err %d\n",
					       err);
...
static ssize_t vdd_store(struct device *dev, struct device_attribute *attr,
			 const char *buf, size_t count)
{
	//struct i2c_client *client = to_i2c_client(dev->parent);
...
static ssize_t vdd_show(struct device *dev, struct device_attribute *attr,
			char *buf)
{
	char buffer[4] = { 0 };
	//int count = 2;
...
static ssize_t led_en_store(struct device *dev, struct device_attribute *attr,
			    const char *buf, size_t count)
{
	//struct i2c_client *client = to_i2c_client(dev->parent);
	//struct ene_6k582_platform_data *platform_data = i2c_get_clientdata(client);
...
static ssize_t led_en_show(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	//struct i2c_client *client = to_i2c_client(dev->parent);
	//struct ene_6k582_platform_data *platform_data = i2c_get_clientdata(client);
			   ...
			   char *buf)
{
	...
	if (((fw_ver & 0xff00) != 0x100) &&
	    (mode != 0xCE)) { //when the ic is in loader mode
		fw_ver = 0;
	}
	return snprintf(buf, PAGE_SIZE, "0x%x\n", fw_ver);
}
...
static ssize_t read_reg_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	struct i2c_client *client = to_i2c_client(dev->parent);
	unsigned char data[4] = { 0 };
...
static ssize_t read_reg_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
			   ...
			   const char *buf, size_t count)
{
	...
	case 1: //6 color rainbow
		...
		for (i = 0; i < 6; i++) {
			...
			err = ene_6k582_write_bytes(client, addr + 0x10,
						    rgb[i * 3 + 1]);
			if (err != 1) {
				printk("[AURA_STATION] ene_6k582_write_bytes:err %d\n",
				       err);
			...
			err = ene_6k582_write_bytes(client, addr + 0x20,
						    rgb[i * 3 + 2]);
			if (err != 1) {
				printk("[AURA_STATION] ene_6k582_write_bytes:err %d\n",
				       err);
	...
	case 2: //static
		...
		for (i = 0; i < 2; i++) {
			...
			err = ene_6k582_write_bytes(client, addr + 2,
						    rgb[i * 3 + 1]);
			if (err != 1) {
				printk("[AURA_STATION] ene_6k582_write_bytes:err %d\n",
				       err);
			...
			err = ene_6k582_write_bytes(client, addr + 4,
						    rgb[i * 3 + 2]);
			if (err != 1) {
				printk("[AURA_STATION] ene_6k582_write_bytes:err %d\n",
				       err);
	...
	case 6: //commet
		...
		err = ene_6k582_write_bytes(client, 0x8402,
					    0xB); //SPEED -10ms/frame
		if (err != 1) {
			printk("[AURA_STATION] ene_6k582_write_bytes:err %d\n",
			       err);
			mutex_unlock(&g_pdata->ene_mutex);
...
static ssize_t led2_on_store(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev->parent);
	//struct ene_6k582_platform_data *platform_data = i2c_get_clientdata(client);
...
static ssize_t led2_on_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	struct i2c_client *client = to_i2c_client(dev->parent);
	int err = 0;
...
static ssize_t sleep_show(struct device *dev, struct device_attribute *attr,
			  char *buf)
{
	struct i2c_client *client = to_i2c_client(dev->parent);
	int err = 0;
...
static ssize_t update_state_store(struct device *dev,
...
static ssize_t update_state_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", g_update_state);
}
...
static struct attribute *pwm_attrs[] = {
	...
	&dev_attr_apply.attr,	     &dev_attr_mode.attr,
	&dev_attr_VDD.attr,	     &dev_attr_fw_update.attr,
	&dev_attr_fw_ver.attr,	     &dev_attr_frame.attr,
	&dev_attr_speed.attr,	     &dev_attr_led_en.attr,
	&dev_attr_Calibration.attr,  &dev_attr_read_reg.attr,
	&dev_attr_reg_debug.attr,    &dev_attr_mode2.attr,
	&dev_attr_led_on.attr,	     &dev_attr_led2_on.attr,
	&dev_attr_sleep.attr,	     &dev_attr_sleep_time.attr,
	&dev_attr_update_state.attr, NULL
};
...
static int ene_6k582_parse_dt(struct device *dev,
			      struct ene_6k582_platform_data *pdata)
{
...
static int ene_6k582_probe(struct i2c_client *client,
			   const struct i2c_device_id *id)
{
	...
	platform_data = devm_kzalloc(&client->dev,
				     sizeof(struct ene_6k582_platform_data),
				     GFP_KERNEL);
	if (!platform_data) {
...
int ene_6k582_suspend(struct device *dev)
{
	...
	//set delay time
	mutex_lock(&g_pdata->ene_mutex);
	err = ene_6k582_write_bytes(client, 0X8454,
				    (delay_time & 0xff00) >> 8); //H
	if (err != 1)
		printk("[AURA_STATION] ene_6k582_write_bytes:err %d\n", err);
