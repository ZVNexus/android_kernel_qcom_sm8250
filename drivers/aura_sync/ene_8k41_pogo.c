...
static ssize_t pogo_show(struct device *dev, struct device_attribute *attr,
			 char *buf)
{
...
static struct attribute *pwm_attrs[] = {
	...
	&dev_attr_blue_pwm.attr,  &dev_attr_apply.attr,
	&dev_attr_mode.attr,	  &dev_attr_VDD.attr,
	&dev_attr_fw_update.attr, &dev_attr_fw_ver.attr,
	&dev_attr_frame.attr,	  &dev_attr_speed.attr,
	&dev_attr_led_en.attr,	  &dev_attr_Calibration.attr,
	&dev_attr_pogo.attr,	  NULL
};
...
static int ene_8k41_fb_callback(struct notifier_block *nb, unsigned long val,
				void *data)
{
	struct ene_8k41_platform_data *platform_data;
	...
	printk("[AURA_POGO] go to the ene_8k41_fb_callback value = %d msm_drm_display_id = %d\n",
	       (int)val, evdata->id);
	platform_data =
		container_of(nb, struct ene_8k41_platform_data, notifier);
