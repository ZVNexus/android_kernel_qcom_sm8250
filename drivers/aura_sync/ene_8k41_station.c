...
static int ene_UpdateFirmware(char *fw_buf)
{
	...
	for (addr = 0x0000; addr <= 0x3F80; addr += 0x80) {
		...
		if (err < 0)
			printk("[AURA_EC] REG[0xF808]: write err %d\n", err);
		err = ene_hid_write_bytes(0xF809,
					  (addr >> 8) & 0xFF); //	Addr_H
		if (err < 0)
			printk("[AURA_EC] REG[0xF809]: write err %d\n", err);
...
static ssize_t ec_show(struct device *dev, struct device_attribute *attr,
		       char *buf)
{
...
static struct attribute *pwm_attrs[] = {
	...
	&dev_attr_blue_pwm.attr, &dev_attr_apply.attr, &dev_attr_mode.attr,
	//	&dev_attr_VDD.attr,
	&dev_attr_fw_update.attr, &dev_attr_fw_ver.attr, &dev_attr_frame.attr,
	&dev_attr_speed.attr, &dev_attr_led_en.attr, &dev_attr_ec.attr, NULL
};
...
static int ene_hid_probe(struct platform_device *pdev)
{
	...
	platform_data =
		kzalloc(sizeof(struct ene_8k41_platform_data), GFP_KERNEL);
