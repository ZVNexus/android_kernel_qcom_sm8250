...
static int proximity_set_threshold(void)
{
	...
	ret = psensor_factory_read_high(PSENSOR_HI_CALIBRATION_FILE_2ND);
	...
	ret = psensor_factory_read_low(PSENSOR_LOW_CALIBRATION_FILE_2ND);	
...
mutex_lock(&g_alsps_lock);
	...
	if(ALSPS_hw_client == NULL)	{
		dbg("2nd ALSPS_hw_client is NULL \n");
		goto ist_err;
	}
...
static int proximity_check_minCT(void)
{
	...
	if(ret >= 0) {
		...
		log("2nd Proximity read INF Calibration : %d\n", g_ps_data->g_ps_calvalue_inf);
	}else{
...
static int __init ALSPS_init_2nd(void)
{
	...
	ALSPS_hw_client = ALSPS_hw_getHardware_2nd();
	if(ALSPS_hw_client == NULL)
		goto init_err;
