...
static int aw8697_write_cali_to_file(unsigned int cali_re, int channel,
				     int type)
{
	struct file *fp;
	char buf[50] = { 0 };
	...
	if (IS_ERR(fp)) {
		pr_err("%s:channel:%d open %s failed!\n", __func__, channel,
		       type ? AWINIC_RTP_CALI_FILE : AWINIC_CALI_FILE);
		return -EINVAL;
	}
...
static int aw8697_get_cali_from_file(unsigned int *cali_re, int channel,
				     int type)
{
	struct file *fp;
	/*struct inode *node;*/
