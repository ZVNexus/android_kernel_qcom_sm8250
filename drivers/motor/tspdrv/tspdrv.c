...
static int g_nTimerPeriodMs =
	5; /* 5ms timer by default. This variable could be used by the SPI.*/
...
#include <ImmVibeSPI.c>
	...
	defined(VIBE_RUNTIME_RECORD)
#include <tspdrvRecorder.c>
#endif

/* Device name and version information */
#define VERSION_STR " v5.4.28.2\n" /* DO NOT CHANGE - this is auto-generated */
#define VERSION_STR_LEN                                                        \
	16 /* account extra space for future extra digits in version number */
static char g_szDeviceName[(VIBE_MAX_DEVICE_NAME_LENGTH + VERSION_STR_LEN) *
...
asmlinkage static void _DbgOutV(int level, const char *fmt, va_list args)
{
	...
	int ret;
	size_t size = sizeof(printk_buf);
	ret = scnprintf(printk_buf, size, KERN_EMERG "%s:%s %s", MODULE_NAME,
			prefix[level], fmt);
	if (ret < size)
		vprintk(printk_buf, args);
}
...
static struct file_operations fops = {
	...
	.release = release,
	.llseek =
		default_llseek /* using default implementation as declared in linux/fs.h */
};
...
static int __init tspdrv_init(void)
{
	...
	DbgOutErr((
		"*** tspdrv: runtime recorder feature is ON for debugging which should be OFF in release version.\n"
...
static ssize_t read(struct file *file, char *buf, size_t count, loff_t *ppos)
{
	const size_t nBufSize =
		(g_cchDeviceName > (size_t)(*ppos)) ?
			min(count, g_cchDeviceName - (size_t)(*ppos)) :
			0;
...
#endif
{
	...
	case TSPDRV_ENABLE_AMP:
		if (bEnable_Vib == 0x1 && arg != 0) {
			DbgOutInfo((
				"tspdrv: %s:TSPDRV_ENABLE_AMP: VIB disable:(bEnable_Vib=0x%02x,index=%d)\n",
	...
	case TSPDRV_SET_DBG_LEVEL: {
		long nDbgLevel;
		if (0 != copy_from_user((void *)&nDbgLevel,
					(const void __user *)arg,
					sizeof(long))) {
			/* Error copying the data */
			DbgOutErr((
				"copy_from_user failed to copy debug level data.\n"));
			return -1;
		}
	...
	case TSPDRV_SET_RUNTIME_RECORD_FLAG: {
		long nRecordFlag;
		if (0 != copy_from_user((void *)&nRecordFlag,
					(const void __user *)arg,
					sizeof(long))) {
			/* Error copying the data */
			DbgOutErr((
				"copy_from_user failed to copy runtime record flag.\n"));
			return -1;
		}
	...
	case TSPDRV_SET_RUNTIME_RECORD_BUF_SIZE: {
		...
		if (0 != copy_from_user((void *)&nRecorderBufSize,
					(const void __user *)arg,
					sizeof(long))) {
			/* Error copying the data */
			DbgOutErr((
				"copy_from_user failed to copy recorder buffer size.\n"));
			return -1;
		}
	...
	case TSPDRV_SET_DEVICE_PARAMETER: {
		...
		if (0 != copy_from_user((void *)&deviceParam,
					(const void __user *)arg,
					sizeof(deviceParam))) {
			/* Error copying the data */
			DbgOutErr((
				"tspdrv: copy_from_user failed to copy kernel parameter data.\n"));
			return -1;
		}
		...
		case VIBE_KP_CFG_UPDATE_RATE_MS:
			...
			g_ktTimerPeriod =
				ktime_set(0, g_nTimerPeriodMs * 1000000);
#endif
			break;
		...
		case VIBE_KP_CFG_FREQUENCY_PARAM6:
			if (0 > ImmVibeSPI_ForceOut_SetFrequency(
					deviceParam.nDeviceIndex,
					deviceParam.nDeviceParamID,
					deviceParam.nDeviceParamValue)) {
				DbgOutErr((
					"tspdrv: cannot set device frequency parameter.\n"));
				return -1;
			}
			break;
		}
