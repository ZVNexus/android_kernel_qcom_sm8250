...
#warning                                                                       \
	"The Kernel does not have high resolution timers enabled. Either provide a non hr-timer implementation of VibeOSKernelLinuxTime.c or re-compile your kernel with CONFIG_HIGH_RES_TIMERS=y"
#endif

#include <linux/hrtimer.h>
#include <linux/semaphore.h>
//ASUS_BSP: wakelock modify for asus build
...
static void VibeOSKernelLinuxStartTimer(void)
{
	...
	if (ImmVibeSPI_ForceOut_BufferFull())
		do
#else
	else
#endif
        ...
        */
			res = down_interruptible(
				&g_hSemaphore); /* wait for the semaphore to be freed by the timer */
			if (res != 0) {
				DbgOutInfo((
					"VibeOSKernelLinuxStartTimer: down_interruptible interrupted by a signal.\n"));
			}
		}
#ifdef IMMVIBESPI_USE_BUFFERFULL
...
static void VibeOSKernelLinuxStopTimer(void)
{
	...
	if (!hrtimer_callback_running(&g_tspTimer)) {
		...
		if (!hrtimer_active(&g_tspTimer) &&
		    VibeSemIsLocked(&g_hSemaphore)) {
			s64 remaining =
				ktime_to_us(hrtimer_get_remaining(&g_tspTimer));
			DbgOutErr((
				"VibeOSKernelLinuxStopTimer: locked but timer inactive (remaining=%ldus watchdog=%d, started=%d)",
				remaining, g_nWatchdogCounter,
				g_bTimerStarted));
		}
		...
		if (processing) {
			...
			DbgOutErr((
				"VibeOSKernelLinuxStopTimer: called stop while processing data started %ldus ago",
				elapsed));
		}
	}
#endif
