...
#define DEFAULT_BOT_VD_CLAMP 5500 //5.5V, gamma
#endif
 ...
 */
static int gVibDebugLog = 0; //0;
//end

//global i2c bus status
//static struct dw791x_priv *dw791x_devs[NUM_ACTUATORS]={NULL, NULL};
static struct dw791x_priv *dw791x_devs[NUM_ACTUATORS] = { NULL };
//#if 0
...
static int dw791x_pinctrl_init(struct i2c_client *i2c,
			       struct dw791x_priv *pdev);
...
static int i2c_recv_buf(struct i2c_client *i2c, unsigned char reg,
...
VibeStatus I2CWriteWithResendOnError_DW7914(struct dw791x_priv *dw791x,
					    VibeUInt16 nBufferSizeInBytes,
					    VibeInt8 *pForceOutputBuffer);
IMMVIBESPIAPI VibeStatus ImmVibeSPI_ForceOut_SetSamples(
	VibeUInt8 nActuatorIndex, VibeUInt16 nOutputSignalBitDepth,
...
** I2C Driver
*/
static int dw791x_probe(struct i2c_client *client,
...
int channel_to_index(int ch)
{
	...
	case 1:
		...
		break;
	default:
		DbgOut((DBL_ERROR, "not support channel=%d\n", ch));
	}
	return index;
}
//===================
...
static const struct i2c_device_id dw791x_drv_id[] = { { "dw791x", 0 }, {} };
...
release  : 2018.02.28
====================================================================================== */
int dw791x_play_mode_sel(struct i2c_client *client, int play_mode, u32 set1,
			 u32 set2, u32 set3)
{
	int ret = 0;
	...
	DbgOut((DBL_INFO, "%s: %s:%d\n, %d, %d, %d", __func__,
		(getActuatorIndex(client) == 0) ? "Bot" : "Top", play_mode,
		set1, set2, set3));
	if (!dw791x->i2c_status) {
		DbgOut((DBL_ERROR, "%s: %d , vibrator %d i2c fail\n", __func__,
			play_mode, getActuatorIndex(client)));
		return -1;
	}
	...
	if (play_mode > 100) {
		...
		if (dw791x->bRTP_Play_Break) {
			if (gVibDebugLog & 0x0010)
				DbgOut((DBL_ERROR,
					"%s:(2) bin_num=%d, break=%d, cancel_work_sync!\n",
					__func__, (play_mode - 100),
					dw791x->bRTP_Play_Break));
	...
	//end
		...
		case 3: // request firmware function prove : page1 point 2
			if (request_firmware(&dw791x->fw, FW_FILE,
					     &dw791x->i2c->dev) == 0) {
				//DbgOut((DBL_INFO, "fw_name= %s fw_size= %ld\n", FW_FILE, dw791x->fw->size));
				request_transfer_mem_wave(
					client, 0, 1, (u8 *)dw791x->fw->data,
					dw791x->fw->size);
			}
...
#else //wave2 play
			dw791x_byte_write(client, 0x0B,
					  0x05); //memory + auto brake
			dw791x_byte_write(client, DW7914_WAVQ1, 2);
			dw791x_byte_write(client, 0x1D,
					  0x03); //brake1 assign with mem3
			dw791x_byte_write(client, 0x1E,
					  0x28); //VD gain & cycle assign
			dw791x_byte_write(client, dw791x->play_back, DRV_PLAY);
#endif
			break;
		...
		case 12: // long-rtp function prove: demo1
			dw791x_bst_option(client, 0, 0, 0); //default
			if (request_firmware(&dw791x->fw, RTP_03_FW_FILE,
					     &dw791x->i2c->dev) == 0) {
				DbgOut((DBL_INFO, "fw_name= %s fw_size= %ld\n",
					RTP_03_FW_FILE, dw791x->fw->size));
				request_transfer_rtp_wave(
					client, (u8 *)dw791x->fw->data, 1);
			}
			release_firmware(dw791x->fw);
			break;
		case 13: // long-rtp function prove: demo1
			dw791x_bst_option(client, 0, 0, 0); //default
			if (request_firmware(&dw791x->fw, RTP_07_FW_FILE,
					     &dw791x->i2c->dev) == 0) {
				DbgOut((DBL_INFO, "fw_name= %s fw_size= %ld\n",
		...
		case 14: // long-rtp function prove: demo1
			dw791x_bst_option(client, 0, 0, 0); //default
			if (request_firmware(&dw791x->fw, RTP_06_FW_FILE,
					     &dw791x->i2c->dev) == 0) {
				DbgOut((DBL_INFO, "fw_name= %s fw_size= %ld\n",
		...
		case 15: // long-rtp function prove: demo1
			dw791x_bst_option(client, 0, 0, 0); //default
			if (request_firmware(&dw791x->fw, RTP_08_FW_FILE,
					     &dw791x->i2c->dev) == 0) {
				DbgOut((DBL_INFO, "fw_name= %s fw_size= %ld\n",
		...
		case 16: // factory default power
			dw791x_bst_option(client, 0, 0, 0); //default
			if (request_firmware(&dw791x->fw, RTP_GUN1_FW_FILE,
					     &dw791x->i2c->dev) == 0) {
				DbgOut((DBL_INFO, "fw_name= %s fw_size= %ld\n",
		...
		case 17: // long-rtp function prove, gun2
			dw791x_bst_option(client, 0, 0, 0); //default
			if (request_firmware(&dw791x->fw, RTP_GUN2_FW_FILE,
					     &dw791x->i2c->dev) == 0) {
				DbgOut((DBL_INFO, "fw_name= %s fw_size= %ld\n",
		...
		case 18: // long-rtp function prove, gun3
			dw791x_bst_option(client, 0, 0, 0); //default
			if (request_firmware(&dw791x->fw, RTP_GUN3_FW_FILE,
					     &dw791x->i2c->dev) == 0) {
				DbgOut((DBL_INFO, "fw_name= %s fw_size= %ld\n",
		...
		case 19: // long-rtp function prove, gun4
			dw791x_bst_option(client, 0, 0, 0); //default
			if (request_firmware(&dw791x->fw, RTP_GUN4_FW_FILE,
					     &dw791x->i2c->dev) == 0) {
				DbgOut((DBL_INFO, "fw_name= %s fw_size= %ld\n",
		...
		case 20: // long-rtp function prove, gun5
			dw791x_bst_option(client, 0, 0, 0); //default
			if (request_firmware(&dw791x->fw, RTP_GUN5_FW_FILE,
					     &dw791x->i2c->dev) == 0) {
				DbgOut((DBL_INFO, "fw_name= %s fw_size= %ld\n",
		...
		case 21: // init for trigger, wave2, only rising
			dw791x_byte_write(
				client, 0x03,
				0x41); // trig priority, trig2 master, trig1 as pulse trigger input
			dw791x_byte_write(client, 0x0B, 0x01); // mem mode
			if (getActuatorIndex(client) ==
			    0) //gamma: trigger2,  trigger tap(mem1), air trigger
			{
			...
			dw791x_byte_write(client, 0x26,
					  dw791x->trigger_r); // trig2_r, mem2
			dw791x_byte_write(client, 0x28,
					  dw791x->break_r); // brake 1 assign
			dw791x_byte_write(client, 0x29,
					  dw791x->trigger_f); // trig2_r, mem2
			dw791x_byte_write(client, 0x2B,
					  dw791x->break_f); // brake 1 assign
			dw791x_byte_write(client, 0x2C,
					  0x28); // VD gain & cycle
			dw791x_byte_write(client, 0x45,
					  0xdd); // TRIG2_R_EN, TRIG2_F_EN
			...
			if (getActuatorIndex(client) == 0) //Tap
			{
				...
				dw791x_seq_write(client, dw791x->mem_input,
						 0x027C, RAM_ADDR16,
						 (u8 *)tap_gamma_pcm1,
						 sizeof(tap_gamma_pcm1));
			}
		...
		case 23:
			dw791x_byte_write(
				...
				0x41); // trig priority, trig2 master, trig1 as pulse trigger input
			dw791x_byte_write(client, 0x0B,
					  0x01); // mem mode + auto_brake
			//0832: trigger3, trigger slider(mem1, 2, 3, 4, 5)
			dw791x_byte_write(
				client, 0x2d,
				0x01); // trig3_r, typing_0832_h4, mem10(0x0A) 46(0x2E)
			dw791x_byte_write(
				client, 0x2f,
				0x00); // brake 1 assign, typing_0832_h5, mem11(00B), 51(0x33)
			dw791x_byte_write(client, 0x30, 0x00); // trig3_f, mem2
			dw791x_byte_write(client, 0x32, 0x00); // brake 1 assign
			dw791x_byte_write(client, 0x33,
					  0x08); // VD gain & cycle
			//use go bit to play
		...
		case 24: // init for trigger1, wave8, only rising
			...
			dw791x_byte_write(
				client, 0x03,
				0x61); // trig priority, trig2 master, trig1 as edge trigger input
			dw791x_byte_write(client, 0x0B, 0x01); // mem mode
			//use go bit to play
			dw791x_byte_write(client, 0x45, 0xdd); // TRIG_DET_EN
			if (getActuatorIndex(client) == 0) //gamma
			{
				dw791x_byte_write(client, 0x1f,
						  0x02); // trig1_r, mem2, 6
				dw791x_byte_write(client, 0x21,
						  0x00); // brake 1 assign
				dw791x_byte_write(client, 0x22,
						  0x00); // trig1_f, 00
				dw791x_byte_write(client, 0x24,
						  0x00); // brake 1 assign
				dw791x_byte_write(client, 0x25,
						  0x28); // VD gain & cycle
				if (set1 == 1) {
					//DbgOut((DBL_INFO, "write homekey_gamma_h1_1 header\n"));
					dw791x_seq_write(
						client, dw791x->mem_input, 0x06,
						RAM_ADDR16,
						(u8 *)homekey_gamma_h1_1, 5);
				} else if (set1 == 3) {
					//DbgOut((DBL_INFO, "write homekey_gamma_h1_3 header\n"));
					dw791x_seq_write(
						client, dw791x->mem_input, 0x06,
						RAM_ADDR16,
						(u8 *)homekey_gamma_h1_3, 5);
				} else if (set1 == 0) {
					//DbgOut((DBL_INFO, "write homekey_gamma_h1_0 header\n"));
					dw791x_seq_write(
						client, dw791x->mem_input, 0x06,
						RAM_ADDR16,
						(u8 *)homekey_gamma_h1_0, 5);
				} else {
					DbgOut((DBL_INFO,
						"write homekey_gamma_h1 header\n"));
					dw791x_seq_write(client,
							 dw791x->mem_input,
							 0x06, RAM_ADDR16,
							 (u8 *)homekey_gamma_h1,
				...
				DbgOut((DBL_INFO,
					"write homekey_gamma_pcm1 data\n"));
				dw791x_seq_write(client, dw791x->mem_input,
						 0x049F, RAM_ADDR16,
						 (u8 *)homekey_gamma_pcm1,
						 sizeof(homekey_gamma_pcm1));
			}
			break;
		case 25: // init for trigger3, only rising
			...
			if (getActuatorIndex(client) == 0) //gamma
			{
				...
				dw791x_byte_write(
					client, 0x2d,
					0x05); // trig3_r, mem5, 21(0x15)
				dw791x_byte_write(client, 0x2f,
						  0x00); // brake 1 assign
				dw791x_byte_write(client, 0x30,
						  0x00); // trig3_f, mem10
				dw791x_byte_write(client, 0x32,
						  0x00); // brake 1 assign
				dw791x_byte_write(client, 0x33,
						  0x28); // VD gain & cycle
				if (set1 == 1) {
					...
					dw791x_seq_write(
						client, dw791x->mem_input, 0x15,
						RAM_ADDR16,
						(u8 *)typing_gamma_h3_1, 5);
				} else if (set1 == 3) {
				...
				dw791x_seq_write(client, dw791x->mem_input,
						 0x0AD8, RAM_ADDR16,
						 (u8 *)typing_gamma_pcm3,
						 sizeof(typing_gamma_pcm3));
			}
			//use go bit to play
			dw791x_byte_write(client, 0x45, 0xdd); // TRIG_DET_EN
			break;
		...
		case 30: // BOT_VIB Max power
			...
			if (request_firmware(&dw791x->fw,
					     RTP_BOT_VIB_MAX_POWER_FW_FILE,
					     &dw791x->i2c->dev) == 0) {
				request_transfer_rtp_wave(
					client, (u8 *)dw791x->fw->data, 1);
			}
		...
		case 31: // BOT_VIB Rated power
			...
			//dw791x_vd_clamp_set(client, 2800);	//2.80V
			if (request_firmware(&dw791x->fw,
					     RTP_BOT_VIB_RATED_POWER_FW_FILE,
					     &dw791x->i2c->dev) == 0) {
				request_transfer_rtp_wave(
					client, (u8 *)dw791x->fw->data, 1);
			}
		...
		case 32: // BOT_VIB Sweep rated power
			...
			if (request_firmware(
				    &dw791x->fw,
				    RTP_BOT_VIB_SWEEP_RATED_POWER_FW_FILE,
				    &dw791x->i2c->dev) == 0) {
				request_transfer_rtp_wave(
					client, (u8 *)dw791x->fw->data, 1);
			}
		...
		case 34: // TOP_VIB Rated power
			...
			//dw791x_vd_clamp_set(client, 2520);	//2.52V
			if (request_firmware(&dw791x->fw,
					     RTP_TOP_VIB_RATED_POWER_FW_FILE,
					     &dw791x->i2c->dev) == 0) {
				request_transfer_rtp_wave(
					client, (u8 *)dw791x->fw->data, 1);
			}
		...
		case 35: // TOP_VIB Sweep rated power
			...
			if (request_firmware(
				    &dw791x->fw,
				    RTP_TOP_VIB_SWEEP_RATED_POWER_FW_FILE,
				    &dw791x->i2c->dev) == 0) {
				request_transfer_rtp_wave(
					client, (u8 *)dw791x->fw->data, 1);
			}
		...
		case 36: // BOT_VIB VD test 500ms
			...
			if (request_firmware(&dw791x->fw,
					     RTP_BOT_VIB_VD_TEST_500_FW_FILE,
					     &dw791x->i2c->dev) == 0) {
				request_transfer_rtp_wave(
					client, (u8 *)dw791x->fw->data, 1);
			}
		...
		case 37: // BOT_VIB VD test 1.5s
			...
			//dw791x_vd_clamp_set(client, 2800);	//2.8V
			if (request_firmware(&dw791x->fw,
					     RTP_BOT_VIB_VD_TEST_1500_FW_FILE,
					     &dw791x->i2c->dev) == 0) {
				request_transfer_rtp_wave(
					client, (u8 *)dw791x->fw->data, 4);
			}
			release_firmware(dw791x->fw);
			break;
		case 38: // TOP_VIB VD test 500ms
			...
			//dw791x_vd_clamp_set(client, 2520);	//2.52V
			if (request_firmware(&dw791x->fw,
					     RTP_TOP_VIB_VD_TEST_500_FW_FILE,
					     &dw791x->i2c->dev) == 0) {
				request_transfer_rtp_wave(
					client, (u8 *)dw791x->fw->data, 1);
			}
		...
		case 39: // TOP_VIB VD test 1.5s
			...
			//dw791x_vd_clamp_set(client, 2520);	//2.52V
			if (request_firmware(&dw791x->fw,
					     RTP_TOP_VIB_VD_TEST_1500_FW_FILE,
					     &dw791x->i2c->dev) == 0) {
				request_transfer_rtp_wave(
					client, (u8 *)dw791x->fw->data, 1);
			}
		...
		case 40: // BOT_VIB VD test 6.67ms
			...
			//dw791x_vd_clamp_set(client, 2800);	//2.8V
			if (request_firmware(&dw791x->fw,
					     RTP_BOT_VIB_VD_TEST_6_FW_FILE,
					     &dw791x->i2c->dev) == 0) {
				request_transfer_rtp_wave(
					client, (u8 *)dw791x->fw->data, 10);
			}
			release_firmware(dw791x->fw);
			break;
...
release  : 2019.04.02
====================================================================================== */
static int dw791x_vibrator_control(struct i2c_client *client, int fun,
				   int param)
{
	int ret = 0;
	...
	case 1: // vd_clamp setting
		...
		if (index == 0) //gamma
		{
			if (param < 0 || param > MAX_BOT_VD_CLAMP) {
				DbgOut((DBL_INFO,
					"%s:fun=0x%04x, param=0x%04x, exceed the MAX(%d), set to default (%d)\n",
					__func__, fun, param, MAX_BOT_VD_CLAMP,
					MAX_BOT_VD_CLAMP));
		...
		} else //0832
		{
			if (param < 0 || param > MAX_TOP_VD_CLAMP) {
				DbgOut((DBL_INFO,
					"%s:fun=0x%04x, param=0x%04x, exceed the MAX(%d), set to default (%d)\n",
					__func__, fun, param, MAX_TOP_VD_CLAMP,
					MAX_TOP_VD_CLAMP));
	...
	case 2: // vib_enable
		bEnable_Vib =
			(param &
			 0x03); //0x01: bot, 0x02: top, 0x03: all ebale, 0x00: all disable
	...
	case 3: // channel
		gChannel =
			(param &
			 0x03); //0x01: bot, 0x02: top, 0x03: all , 0x00: none
	...
	case 5: // rtp play
	{
		...
		if (gVibDebugLog & 0x0010) //store node debug
			DbgOut((DBL_INFO,
				"%s:(3) bin_num=%d, break=%d, request_firmware...\n",
				__func__, dw791x->bin_num,
				dw791x->bRTP_Play_Break));
		if (request_firmware(&dw791x->fw, dw791x->fwName,
				     &dw791x->i2c->dev) == 0) {
			if (gVibDebugLog & 0x0010) //check break status
				DbgOut((DBL_INFO,
					"%s: (4)fw_name= %s ,bin_num=%d, repeat=%d, break=%d, queuework...\n",
					__func__, dw791x->fwName,
					dw791x->bin_num, dw791x->repeat,
					dw791x->bRTP_Play_Break));
			queue_work(dw791x->rtp_workqueue, &dw791x->rtp_work);
		} else {
			DbgOut((DBL_INFO,
				"%s: (4)fw_name= %s ,bin_num=%d, repeat=%d, break=%d, request_firmware failed!\n",
				__func__, dw791x->fwName, dw791x->bin_num,
				dw791x->repeat, dw791x->bRTP_Play_Break));
		}
	} break;
	...
	case 8: //trigger pin setting, called by init.asus.boot_vibrate.sh
		DbgOut((DBL_INFO, "%s:fun=0x%04x:trigger1 setting\n", __func__,
			fun));
		dw791x_play_mode_sel(client, 24, gTrigger1_3_Level, 0,
				     0); //trigger1
		...
		DbgOut((DBL_INFO, "%s:fun=0x%04x:trigger3 setting\n", __func__,
			fun));
		dw791x_play_mode_sel(client, 25, gTrigger1_3_Level, 0,
				     0); //trigger3
...
static int init_gpio(struct dw791x_priv *dw791x)
{
	...
	//bot_trigger3-pin
#if 1
	if (gpio_request(dw791x->bot_trigger3_gpio, "dw791x_trigger_gpio")) {
		DbgOut((DBL_INFO, "%s:dw791x_trigger_gpio request faiure: %d\n",
	...
	if (ret < 0) {
		DbgOut((DBL_INFO,
			"%s:Can't set dw791x_trigger_gpio direction, error %i\n",
			__func__, ret));
		gpio_free(dw791x->bot_trigger3_gpio);
		return -EINVAL;
	} else {
		DbgOut((DBL_INFO,
			"%s:dw791x_trigger_gpio direction output: %d\n",
			__func__, dw791x->bot_trigger3_gpio));
...
static void bot_vib_irq_rtp_work(struct work_struct *work)
{
	...
	if (!dw791x->i2c_status) {
		...
		if (rx[0] & 0x0f) {
			DbgOut((DBL_INFO,
				"%s:VIB:status0:0x%02x, status1:0x%02x\n",
				__func__, rx[0], rx[1]));
			if (rx[0] & 0x01)
				DbgOut((DBL_INFO,
					"%s:VIB:SCP (Short current is detected in VBST. The device goes into standby)\n",
					__func__));
			if (rx[0] & 0x02)
				DbgOut((DBL_INFO,
					"%s:VIB:OCP (Over-current is detected in OUTP or OUTN. The device goes into standby)\n",
					__func__));
			if (rx[0] & 0x04)
				DbgOut((DBL_INFO,
					"%s:VIB:TSD (Temp is above over-temperature threshold. The device goes into standby)\n",
					__func__));
			if (rx[0] & 0x08)
				DbgOut((DBL_INFO,
					"%s:VIB:UVLO (A VIN droop was observed. The device goes into standby)\n",
					__func__));
		}
		//end
		if (gVibDebugLog & 0x0001) //enable irq message
		{
			...
			rx[1] = dw791x_byte_read(dw791x->i2c, DW7914_STATUS1);
			DbgOut((DBL_INFO,
				"%s:TOP_VIB:status0:0x%02x, status1:0x%02x\n",
				__func__, rx[0], rx[1]));
			if (rx[0] & 0x20) {
				//irq_rtp_play_func(0);
				DbgOut((DBL_INFO,
					"TOP_VIB: fifo empty falling edge"));
			}
		}
...
static void bot_vib_rtp_play_work_func(struct work_struct *work)
{
	...
	if (gVibDebugLog & 0x0010) //check break status
		DbgOut((DBL_INFO,
			"%s: (1)fw_name= %s ,bin_num=%d, repeat=%d, break=%d, call request_transfer_rtp_wave()...\n",
			__func__, dw791x->fwName, dw791x->bin_num,
			dw791x->repeat, dw791x->bRTP_Play_Break));
	request_transfer_rtp_wave(dw791x->i2c, (u8 *)dw791x->fw->data,
				  dw791x->repeat);
	//}
	if (dw791x->fw != NULL)
		release_firmware(dw791x->fw);
	...
	if (gVibDebugLog & 0x0010) //store node debug
		DbgOut((DBL_INFO,
			"%s: (2)fw_name= %s ,bin_num=%d, repeat=%d, break=%d ---\n",
			__func__, dw791x->fwName, dw791x->bin_num,
			dw791x->repeat, dw791x->bRTP_Play_Break));
}
...
{
	queue_work(rtp_workqueue[0], &bot_vib_work);
...
static int init_irq_rtp(struct dw791x_priv *pdev)
{
	...
	if (ret < 0) {
		DbgOut((DBL_INFO,
			"BOT_VIB: Can't set GPIO direction, error %i\n", ret));
		gpio_free(pdev->bot_vib_irq_gpio);
		return -EINVAL;
	} else
		DbgOut((DBL_INFO, "BOT_VIB:GPIO direction input: %d\n",
			pdev->bot_vib_irq_gpio));
	...
	ret = request_irq(pdev->bot_vib_irq_num,
			  (irq_handler_t)bot_vib_irq_rtp_handler,
			  IRQF_TRIGGER_FALLING, "bot_vib_trig_interrupt", NULL);
	...
	rtp_workqueue[0] =
		create_singlethread_workqueue("bot_vib_rtp_work_queue");
	return ret;
}
...
static int irq_rtp_exit(int index)
{
	...
	//#else
	struct dw791x_priv *dw791x =
		dw791x_devs[0]; //only top_vib hold all gpio information
	//#endif
...
static void vibrator_set_trigger(int value)
{
	...
	if (gTrigger_gpio > 0) {
		...
		if (ret < 0)
			DbgOut((DBL_INFO,
				"%s:Can't set trigger_gpio out failed: %i\n",
				__func__, ret));
		if (gVibDebugLog & 0x0010) //store node debug
...
static ssize_t bot_vib_control_set(struct device *dev,
				   ...
				   const char *buf, size_t count)
{
	...
	DbgOut((DBL_INFO,
		"%s cmd: bus=%d, mode=0x%08x, fun=0x%04x, param=0x%04x\n",
		__func__, bus, mode, (mode & 0xffff0000) >> 16,
		(mode & 0x0000ffff)));
	dw791x_vibrator_control(
		dw791x->i2c, (mode & 0xffff0000) >> 16,
		(mode & 0x0000ffff)); //this function has i2c failure detection
	//DbgOut((DBL_INFO, "%s cmd: bus=%d, buf=%s\n", __func__, bus, buf));
	return count;
}

static ssize_t bot_vib_control_show(struct device *dev,
...
DEVICE_ATTR(bot_vib_control, (S_IWUSR | S_IRUGO), bot_vib_control_show,
	    bot_vib_control_set);
...
static int dw791x_vibrator_control_sysfs_init(void)
{
	...
	ret = sysfs_create_file(android_vibrator_kobj,
				&dev_attr_bot_vib_control.attr);
	if (ret) {
		DbgOut((DBL_INFO,
			"%s: sysfs_create_file bot_vib_control failed\n",
			__func__));
	} else
		DbgOut((DBL_INFO,
			"attribute bot_vib_control file register Done"));
...
static int dw791x_parse_dt(struct i2c_client *i2c, struct dw791x_priv *pdev)
{
	...
	//bot_vib_irq-gpio
	DbgOut((DBL_INFO, "parse irq-gpio\n"));
	pdev->bot_vib_irq_gpio = of_get_named_gpio(np, "dw791x,irq-gpio", 0);

	if (pdev->bot_vib_irq_gpio < 0) {
		printk("%s: Looking up %s property in node %s failed %d\n",
		       __func__, "dw791x,irq-gpio", dev->of_node->full_name,
		       pdev->bot_vib_irq_gpio);
...
#if 1
	DbgOut((DBL_INFO, "%s:parse dw791x_trigger_gpio\n", __func__));
	pdev->bot_trigger3_gpio =
		of_get_named_gpio(np, "dw791x_trigger_gpio", 0);

	if (pdev->bot_trigger3_gpio < 0) {
		printk("%s: Looking up %s property in node %s failed %d\n",
		       __func__, "dw791x_trigger_gpio", dev->of_node->full_name,
		       pdev->bot_trigger3_gpio);
		pdev->bot_trigger3_gpio = -1;
	} else {
		printk(KERN_ERR "%s: get dw791x_trigger_gpio(%u)\n", __func__,
		       pdev->bot_trigger3_gpio);
		//gTrigger_gpio=pdev->bot_trigger3_gpio;
	}
	if (!gpio_is_valid(pdev->bot_trigger3_gpio)) {
		printk(KERN_ERR "%s: dw791x_trigger_gpio(%u) is invalid\n",
		       __func__, pdev->bot_trigger3_gpio);
	} else
		printk(KERN_ERR "%s: dw791x_trigger_gpio(%u) is valid\n",
		       __func__, pdev->bot_trigger3_gpio);
#endif
...
static enum led_brightness dw791x_brightness_get(struct led_classdev *cdev)
{
	return 0;
}
...
static ssize_t debuglog_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	int ret;
	int val;
			...
			char *buf)
{
	...
	index = channel_to_index(gChannel);
	if (index == -1)
		return -1;
			      ...
			      const char *buf, size_t count)
{
	...
	if (rw == 0x00) //read register
	{
		...
		DbgOut((DBL_INFO,
			"%s:%s: write value(0x%02x) to register(0x%02x)!\n",
			__func__, (index == 0) ? "BOT_VIB" : "TOP_VIB", value,
			addr));
				  ...
				  const char *buf, size_t count)
{
	...
	dw791x_PWM_set(dw791x->i2c, gPWM);
	dw791x_dump_registers(dw791x);
	dw791x_play_mode_sel(dw791x->i2c, 24, gTrigger1_3_Level, 0,
			     0); //trigger1
...
//trigger_waveform
static ssize_t trigger_waveform_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	...
	DbgOut((DBL_INFO, "trigger2 pattern: %s %d %d %d %d\n", "Gamma",
		dw791x_1->trigger_r, dw791x_1->break_r, dw791x_1->trigger_f,
		dw791x_1->break_f));

	return snprintf(buf, PAGE_SIZE,
			"[trigger2 pattern]\nGamma: %d %d %d %d\n",
			dw791x_1->trigger_r, dw791x_1->break_r,
			dw791x_1->trigger_f, dw791x_1->break_f);
}
				      ...
				      const char *buf, size_t count)
{
	...
	*/
	struct dw791x_priv *dw791x = NULL;
	int actuator = 0, trigger_r = 0, break_r = 0, trigger_f = 0,
	    break_f = 0;

	sscanf(buf, "%d %d %d %d %d", &actuator, &trigger_r, &break_r,
	       &trigger_f, &break_f);
	actuator = 0;
	dw791x = dw791x_devs[0];
	...
	DbgOut((DBL_INFO, "trigger2 pattern: %s %d %d %d %d\n",
		(actuator == 0) ? "Gamma" : "0832", dw791x->trigger_r,
		dw791x->break_r, dw791x->trigger_f, dw791x->break_f));
	return count;
}
...
void dw7914_trigger3_gpio(int channel, bool enable)
{
	...
	//#if BOT_IS_INDEX_0
	dw791x = dw791x_devs[0]; //all gpio info in dw791x_devs[1]
	//#else
	...
	if (channel & 0x01) { //gamma
		...
		if (!dw791x || !dw791x->i2c_status) {
			...
			if (level == 0) {
				...
				dw791x_seq_write(dw791x->i2c, dw791x->mem_input,
						 0x01, RAM_ADDR16,
						 (u8 *)tap_gamma_h1_0, 5);
			} else if (level == 1) {
				//DbgOut((DBL_INFO, "write tap_gamma_h1_1 header\n"));
	...
	if (channel & 0x01) { //gamma
		...
		if (!dw791x->i2c_status) {
			...
			if (level == 1) {
				...
				dw791x_seq_write(client, dw791x->mem_input,
						 0x0B, RAM_ADDR16,
						 (u8 *)typing_gamma_h1_1, 5);
				//DbgOut((DBL_INFO, "write typing_gamma_h2_1 header\n"));
				dw791x_seq_write(client, dw791x->mem_input,
						 0x10, RAM_ADDR16,
						 (u8 *)typing_gamma_h2_1, 5);
				//DbgOut((DBL_INFO, "write typing_gamma_h3_1 header\n"));
				dw791x_seq_write(client, dw791x->mem_input,
						 0x15, RAM_ADDR16,
						 (u8 *)typing_gamma_h3_1, 5);
				...
				dw791x_seq_write(client, dw791x->mem_input,
						 0x0B, RAM_ADDR16,
						 (u8 *)typing_gamma_h1_3, 5);
				//DbgOut((DBL_INFO, "write typing_gamma_h2_3 header\n"));
				dw791x_seq_write(client, dw791x->mem_input,
						 0x10, RAM_ADDR16,
						 (u8 *)typing_gamma_h2_3, 5);
				//DbgOut((DBL_INFO, "write typing_gamma_h3_3 header\n"));
				dw791x_seq_write(client, dw791x->mem_input,
						 0x15, RAM_ADDR16,
						 (u8 *)typing_gamma_h3_3, 5);
				...
				dw791x_seq_write(client, dw791x->mem_input,
						 0x0B, RAM_ADDR16,
						 (u8 *)typing_gamma_h1_0, 5);
				//DbgOut((DBL_INFO, "write typing_gamma_h2_0 header\n"));
				dw791x_seq_write(client, dw791x->mem_input,
						 0x10, RAM_ADDR16,
						 (u8 *)typing_gamma_h2_0, 5);
				//DbgOut((DBL_INFO, "write typing_gamma_h3_0 header\n"));
				dw791x_seq_write(client, dw791x->mem_input,
						 0x15, RAM_ADDR16,
						 (u8 *)typing_gamma_h3_0, 5);
				...
				dw791x_seq_write(client, dw791x->mem_input,
						 0x0B, RAM_ADDR16,
						 (u8 *)typing_gamma_h1, 5);
				//DbgOut((DBL_INFO, "write typing_gamma_h2 header\n"));
				dw791x_seq_write(client, dw791x->mem_input,
						 0x10, RAM_ADDR16,
						 (u8 *)typing_gamma_h2, 5);
				//DbgOut((DBL_INFO, "write typing_gamma_h3 header\n"));
				dw791x_seq_write(client, dw791x->mem_input,
						 0x15, RAM_ADDR16,
						 (u8 *)typing_gamma_h3, 5);
			}
		}
	}
	/*else if(channel&0x02){ //0832
	...
	if (trigger == 1) { //gamma & 0832 use the same boot_vib_gpio
		...
		if (channel & 0x01) //gamma
		{
			...
			if (!dw791x->i2c_status) {
				...
				if (gVibDebugLog & 0x0010) //store node debug
					DbgOut((DBL_INFO,
						"%s:trigger=%d (gpio=%d), val=%d, level=%d\n",
						__func__, trigger,
						gTrigger_gpio, val, level));
				//vibrator_set_trigger(val);
				vibrator_set_trigger(1);
				vibrator_set_trigger(0);
			}
...
//mem mode play test
static ssize_t mem_play_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	int ch = -1, waveform_num = 0, rx[2], index = -1;
	struct dw791x_priv *dw791x = NULL;

	DbgOut((DBL_INFO, "%s: +++\n", __func__));

	sscanf(buf, "%d %d", &ch, &waveform_num);
	index = channel_to_index(ch);
	if (index == -1)
		return -1;

	if (waveform_num < 1 || waveform_num > 127)
		waveform_num = 1;
	dw791x = dw791x_devs[index];
...
//rtp mode play test
static ssize_t rtp_play_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	int channel = 1, val = -1;
	struct dw791x_priv *dw791x = NULL;
	...
	if (channel & 0x01) //gamma
	{
		...
		dw791x_vibrator_control(
			dw791x->i2c, 5,
			val); //this function has i2c failure detection
	}
	/*if(channel&0x02)//0832
	{
		dw791x=dw791x_devs[1];
				     ...
				     const char *buf, size_t count)
{
	...
	if (channel & 0x01) //gamma
	{
		...
		if (dw791x->bRTP_Play_Break) {
			if (gVibDebugLog & 0x0010) //store node debug
				DbgOut((DBL_ERROR,
					"%s:(2-1) bin_num=%d, break=%d, cancel_work_sync!\n",
					__func__, val,
					dw791x->bRTP_Play_Break));
			cancel_work_sync(&dw791x->rtp_work);
			dw791x->bRTP_Play_Break = false;
			if (gVibDebugLog & 0x0010) //store node debug
				DbgOut((DBL_ERROR,
					"%s:(2-2) bin_num=%d, break=%d, cancel_work_sync!\n",
					__func__, val,
					dw791x->bRTP_Play_Break));
		}
		//end
		...
		if (gVibDebugLog & 0x0010) //store node debug
			DbgOut((DBL_INFO,
				"%s:(3) bin_num=%d, fwName=%s, repeat=%d, break=%d, request_firmware...\n",
				__func__, dw791x->bin_num, dw791x->fwName,
				repeat, dw791x->bRTP_Play_Break));
		if (request_firmware(&dw791x->fw, dw791x->fwName,
				     &dw791x->i2c->dev) == 0) {
			if (gVibDebugLog & 0x0010) //check break status
				DbgOut((DBL_INFO,
	...
	/*	if(channel&0x02)//0832
	{
		...
		if(request_firmware(&dw791x->fw, fwName, &dw791x->i2c->dev) == 0) {
			if(gVibDebugLog&0x0010)//store node debug
				DbgOut((DBL_INFO, "%s:fw_name= %s fw_size= %ld\n", __func__, fwName, dw791x->fw->size));
			dw791x->bRTP_Play_Break=false;
...
static ssize_t rtp_break_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	struct dw791x_priv *dw791x_gamma; //, *dw791x_0832;
	dw791x_gamma = dw791x_devs[0];
	//dw791x_0832=dw791x_devs[1];
	return snprintf(buf, PAGE_SIZE, "gamma_rtp_break=%s\n",
			(dw791x_gamma->bRTP_Play_Break == true) ? "true" :
								  "false");
}
			       ...
			       size_t count)
{
	...
	if (channel & 0x01) {
		...
		if (!dw791x->i2c_status) {
			...
			if (dw791x->bRTP_Play_Break) {
				dw791x_byte_write(dw791x->i2c,
						  dw791x->play_back, DRV_STOP);
				cancel_work_sync(&dw791x->rtp_work);
			}
			if (gVibDebugLog & 0x0010) //store node debug
				DbgOut((DBL_INFO, "%s:set plackbreak=%s\n",
					__func__,
					(dw791x->bRTP_Play_Break != 0) ?
						"true" :
						"false"));
		}
	}
...
//{
//}
static ssize_t vd_clamp_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	//read vd_clamp
	int val = 0, index = -1;
...
static ssize_t vd_clamp_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	int channel = 1;
	int val = -1, val2 = 0;
	...
	if (channel & 0x01) //gamma
	{
		...
		if (val <= 0 || val > MAX_BOT_VD_CLAMP) {
			DbgOut((DBL_ERROR,
				"%s:val=%d, exceed the MAX(%d), set to default (%d)\n",
				__func__, val, MAX_BOT_VD_CLAMP,
				MAX_BOT_VD_CLAMP));
		...
		if (gVibDebugLog & 0x0010) //store node debug
		{
			...
			DbgOut((DBL_ERROR,
				"%s:read back: channel=%d, vmax(vd_clamp)=%d\n",
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "index=%d, bst_option=%d\n", index,
			val);
}
				...
				size_t count)
{
	...
	DbgOut((DBL_INFO, "%s:index=%d, lv=%d limit=%d ofs=%d\n", __func__,
		index, lv, limit, ofs));
	dw791x_bst_option(dw791x->i2c, lv, limit, ofs); //default
	//read back to check
	val = dw791x_byte_read(dw791x->i2c, DW7914_BST_OPTION);
	DbgOut((DBL_INFO, "%s:read index=%d, vmax(vd_clamp)=%d\n", __func__,
		index, val));
	return count;
}
...
//LED vib callback
//==================================
//static void led_vib_timer_callback(struct work_struct *work)
static enum hrtimer_restart led_vib_timer_callback(struct hrtimer *timer)
{
	struct dw791x_priv *dw791x =
		container_of(timer, struct dw791x_priv, led_vib_timer);
	//struct dw791x_priv *dw791x =dw791x_devs[0];
	//stop vibration
	dw791x->activate = 0;
	dw791x_byte_write(dw791x->i2c, dw791x->play_back, DRV_STOP);
	DbgOut((DBL_ERROR, "%s: led vib timeout to stop.\n"));

	return HRTIMER_NORESTART;
}

void led_vib_timer_start(int index)
{
	struct dw791x_priv *dw791x = dw791x_devs[index];
	...
	if (hrtimer_active(&dw791x->led_vib_timer))
		hrtimer_cancel(&dw791x->led_vib_timer);

	hrtimer_start(&dw791x->led_vib_timer,
		      ktime_set(dw791x->duration / 1000,
				(dw791x->duration % 1000) * 1000000),
		      HRTIMER_MODE_REL);
			     ...
			     char *buf)
{
	...
	dw791x = dw791x_devs[index];
	//read activate
	return snprintf(buf, PAGE_SIZE, "Channel=0x%0x, activate:%d\n",
			      ...
			      const char *buf, size_t count)
{
	...
	if (buf[0] == '1') {
		...
		dw7914_autovib_set(dw791x->i2c);
		dw791x_byte_write(dw791x->i2c, dw791x->play_back, DRV_PLAY);
		led_vib_timer_start(index);
	}
	return count;
}
			     ...
			     char *buf)
{
	...
	dw791x = dw791x_devs[index];
	//read duration
	return snprintf(buf, PAGE_SIZE, "Channel=0x%0x, %d ms\n", gChannel,
			dw791x->duration);
}
...
static struct device_attribute dw791x_haptics_attrs[] = {
	__ATTR(activate, 0664, activate_show,
	       activate_store), //for LED vibrator HAL 1.0
	__ATTR(duration, 0664, duration_show,
	       duration_store), //for LED vibrator HAL 1.0
	__ATTR(state, 0664, state_show, state_store), //for LED vibrator HAL 1.0
	...
	__ATTR(channel, 0664, channel_show, channel_store), //hidl, ims use
	__ATTR(vd_clamp, 0664, vd_clamp_show, vd_clamp_store), //hidl, ims use
	...
	__ATTR(mem_play, 0664, NULL, mem_play_store), //hidl
	__ATTR(trigger_gpio, 0664, NULL, trigger_gpio_store), //hidl
	__ATTR(rtp_break, 0664, rtp_break_show,
	       rtp_break_store), //hidl, ims use
	__ATTR(register, 0664, register_show, register_store),
	__ATTR(vib_enable, 0664, vib_enable_show, vib_enable_store), //hidl
	__ATTR(rtp_play, 0664, NULL, rtp_play_store), //hidl, ims use
	__ATTR(debuglog, 0664, debuglog_show, debuglog_store),
	__ATTR(resample_control, 0664, resample_control_show,
	       resample_control_store),
	//	__ATTR(enable_gpio, 0664, enable_gpio_show, enable_gpio_store),//hidl
	__ATTR(rtp_play_repeat, 0664, NULL, rtp_play_repeat_store),
	__ATTR(trigger_level, 0664, NULL, trigger_level_store),
	__ATTR(trigger_waveform, 0664, trigger_waveform_show,
	       trigger_waveform_store), //hidl
			...
			const struct i2c_device_id *id)
{
	...
	if (!dw791x) {
		//return -ENOMEM;
		DbgOut((DBL_ERROR,
			"%s:dw791x allocate failed, bus=%d, addr=0x%0x ---\n",
			__func__, bus, addr));
		return 0;
	}
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		DbgOut((DBL_ERROR,
			"dw791x_probe: i2c_check_functionality failed.\n"));
		//return -ENODEV;
	}
	...
	if (dw791x->i2c) {
		...
		if (2 != i2c_recv_buf(client, DW791x_PID, &dw791x->chipid, 1)) {
			DbgOut((DBL_ERROR,
				"dw791x_probe: failed to read chipid.\n"));
			dw791x->i2c_status = 0;
			/*return -ENODEV;*/
		} else
			dw791x->i2c_status = 1;
		...
		dw791x->rtp_workqueue = create_singlethread_workqueue(
			"bot_vib_rtp_play_work_queue");
		...
		if (!gVibIF) {
			...
			result = devm_led_classdev_register(&dw791x->i2c->dev,
							    &dw791x->cdev);
			if (result < 0) {
				DbgOut((DBL_ERROR,
					"%s: devm_led_classdev_register failed\n",
					__func__));
			}
			//dw7914 post initial
			dw791x->activate = 0;
			...
			//=======================================
			hrtimer_init(&dw791x->led_vib_timer, CLOCK_MONOTONIC,
				     HRTIMER_MODE_REL);
			dw791x->led_vib_timer.function = led_vib_timer_callback;
			//=======================================
			//end

			//create sysfile
			for (i = 0; i < ARRAY_SIZE(dw791x_haptics_attrs); i++) {
				result = sysfs_create_file(
					&dw791x->cdev.dev->kobj,
					&dw791x_haptics_attrs[i].attr);
				if (result < 0) {
					DbgOut((DBL_ERROR,
						"nrmo1210 sysfs create failed\n"));
				}
			}
		}
		//end
		...
		result = i2c_recv_buf(dw791x->i2c, DW7914_STATUS0, &status,
				      sizeof(status));
		if (result != 2) {
			DbgOut((DBL_ERROR,
				"dw791x_probe: i2c read status0 failure. result=%d\n",
				result));
		}
		result = i2c_recv_buf(dw791x->i2c, DW7914_STATUS1, &status,
				      sizeof(status));
		if (result != 2) {
			DbgOut((DBL_ERROR,
				"dw791x_probe: i2c read status1 failure. result=%d\n",
				result));
		}
...
====================================================================================== */
#ifdef BST_OPT
static void dw791x_bst_option(struct i2c_client *client, u32 lv, u32 limit,
			      u32 ofs)
{
	u8 set = 0;
	int ret = 0;
...
static void dw791x_fifo_size_set(struct i2c_client *client, u32 size)
{
	...
	if (ret < 0)
		DbgOut((DBL_INFO, "%s i2c fifo size set fail.!\n",
			dw791x->dev_name));
}
...
static void dw7914_trigctrl_set(struct i2c_client *client, u8 type, u8 bitset)
{
	...
	if (ret < 0)
		DbgOut((DBL_INFO, "i2c trig register set fail.!\n"));
}
#endif
...
static int dw7914_checksum(struct i2c_client *client, u32 type, u32 page)
{
	...
	if (request_firmware(&dw791x->fw, FW_FILE, &dw791x->i2c->dev) == 0) {
		...
		rx[1] = p[point - 2];
		rx[0] = p[point - 1];
		dw791x->checksum = (u32)rx[3] << 24 | (u32)rx[2] << 16 |
				   (u32)rx[1] << 8 | (u32)rx[0] << 0;
	...
	gsum = (u32)rx[3] << 24 | (u32)rx[2] << 16 | (u32)rx[1] << 8 |
	       (u32)rx[0] << 0;
...
release  : 2018.02.28
====================================================================================== */
static int dw791x_seq_write(struct i2c_client *client, u32 addr, u32 ram_addr,
			    u32 ram_bit, u8 *data, u32 size)
{
	int ret = 0;
	struct i2c_msg xfer[1];
	struct i2c_client *i2c_fnc = client;
	...
	if (size > 1027) {
		ret = -1;
		DbgOut((DBL_INFO,
			"%s:The transferable size has been exceeded.\n",
			__func__));
		return ret;
	}
...
static int transfer_atuovib_wave(struct i2c_client *client, u8 *wave)
{
	...
	if (!dw791x->i2c_status) {
		DbgOut((DBL_INFO,
			"transfer_atuovib_wave vibrator %d i2c fail \n",
			getActuatorIndex(client)));
		return -1;
	}
	...
	dw791x_seq_write(client, dw791x->mem_input, set_addr, RAM_ADDR16,
			 (u8 *)wave + 2, 5);
	...
	for (i = 0; i < loop; i++) {
		...
		dw791x_seq_write(client, dw791x->mem_input, start_addr,
				 RAM_ADDR16, (u8 *)wave + 7 + i * trans_size,
				 trans_size);
	}
	...
	if (tail > 0) {
		...
		dw791x_seq_write(client, dw791x->mem_input, start_addr,
				 RAM_ADDR16, (u8 *)wave + 7 + loop * trans_size,
				 tail);
	}
...
release  : 2018.04.17
====================================================================================== */
static int request_transfer_rtp_wave(struct i2c_client *client, u8 *wave,
				     u32 repeat)
{
	u8 rx[2];
	...
	if (!dw791x->i2c_status) {
		DbgOut((DBL_ERROR,
			"request_transfer_rtp_wave vibrator %d i2c fail!\n",
			getActuatorIndex(client)));
		return -1;
	}
	if (dw791x->bRTP_Play_Break) {
		DbgOut((DBL_ERROR,
			"(%s) (0) (%s)***An event occurred during rtp operation! bRTP_Play_Break = true***\n",
	...
	//This is delay time between the current vibe and the next vibe by trigger input.
	//bit[3-1]: 010 : 10ms
	dw791x_byte_write(client, 0x03, 0x04); // jks ---

	run = curcnt = empty_lock = fifo_level = 0;
	play_size = (u32)wave[1] << 24 | (u32)wave[2] << 16 |
		    (u32)wave[3] << 8 | (u32)wave[4] << 0;
	...
	rx[0] = dw791x_byte_read(client, dw791x->fifo_addrh); //0x49
	rx[1] = dw791x_byte_read(client, dw791x->fifo_addrl); //0x4a
	fifo_size = SRAM_BASE - ((u32)rx[0] << 8 | rx[1]);
	fifo_level = dw791x_byte_read(
		client, DW7914_FIFO_LEVEL_SETH); //0x4b fifo size??
	...
	DbgOut((DBL_INFO,
		"I2C_TRANS_RTP=%d, fifo_size=%d, fifo_level=%d(%d)bytes\n",
		I2C_TRANS_RTP, fifo_size, fifo_level, fifo_level * 256));
	...
	if (!dw791x->bRTP_Play_Break && play_size < I2C_TRANS_RTP) {
		if (gVibDebugLog & 0x0010) //store node debug
			DbgOut((DBL_ERROR,
				"%s: (%s) (%s)write to fifo, play_size: %d\n",
				__func__, (index == 0) ? "Bot_VIB" : "Top_VIB",
				dw791x->fwName, play_size));

		ret = dw791x_seq_write(client, dw791x->rtp_input, 0, RAM_ADDR0,
				       (unsigned char *)wave + 5, play_size);
		if (ret < 0)
			DbgOut((DBL_ERROR,
				"(%s)write to fifo failed!! Code:%d\n",
		...
		if (ret < 0)
			DbgOut((DBL_ERROR, "(%s)go play failed!! Code:%d\n",
				(index == 0) ? "Top_VIB" : "Bot_VIB", ret));
		return ret;
	}
	//dw791x_dump_registers(dw791x);
	...
	while (!run) {
		if (dw791x->bRTP_Play_Break) {
			DbgOut((DBL_ERROR,
				"(%s) (2) (%s)***An event occurred during rtp operation! bRTP_Play_Break = true***\n",
				(index == 0) ? "Bot_VIB" : "Top_VIB",
				dw791x->fwName));
			return -1;
		}
		// step 3: ------------------ fifo level check
		rx[0] = dw791x_byte_read(client, 0x4D); // current fifo status
		...
		if ((fifo_size - fifo_level) > I2C_TRANS_RTP) {
			...
			trans_size = 0; //fifo_size - fifo_level;
		}
		...
		if (curcnt < play_size) {
			...
			if (trans_size > 0) {
				if (dw791x->bRTP_Play_Break) {
					DbgOut((DBL_ERROR,
						"(%s) (3) (%s)***An event occurred during rtp operation! bRTP_Play_Break = true***\n",
						(index == 0) ? "Bot_VIB" :
							       "Top_VIB",
						dw791x->fwName));
					return -1;
				...
				if (gVibDebugLog & 0x0010) //store node debug
					DbgOut((DBL_ERROR, "(fifo=%d trans=%d)",
						fifo_level, trans_size));

				ret = dw791x_seq_write(
					client, dw791x->rtp_input, 0, 0,
					(unsigned char *)wave + 5 + curcnt,
					trans_size); // fifo full write
				if (ret < 0)
					DbgOut((DBL_ERROR,
						"(%s)i2c error!! Code:%d\n",
						(index == 0) ? "Top_VIB" :
			...
			if (!dw791x->bRTP_Play_Break && empty_lock == 0) {
				...
				if (gVibDebugLog & 0x0010) //store node debug
					DbgOut((DBL_ERROR,
						"(%s) fifo_level=%d, fifo_size/2=%d, play_size/2=%d",
						__func__, fifo_level,
						fifo_size / 2,
						(play_size / 3)));
				if (fifo_level > (fifo_size / 3) ||
				    (fifo_level > (play_size / 3))) {
					empty_lock = 1;
					if (gVibDebugLog &
					    0x0010) //store node debug
						DbgOut((DBL_ERROR,
					...
					ret = (0x01 &
					       dw791x_byte_read(
						       client, DW7914_STATUS1));
					if (!ret) {
						DbgOut((DBL_ERROR,
							"%s: Set Go bit and Play, again ...\n",
							__func__));
			...
			} else if (fifo_level == I2C_TRANS_RTP) {
				DbgOut((DBL_ERROR,
					"%s: toggle Go bit for fifo is cleared. \n",
					__func__));
				dw791x_byte_write(client, dw791x->play_back,
						  DRV_PLAY);
			}

			if (dw791x->bRTP_Play_Break) { //if want to stop playing
				DbgOut((DBL_ERROR,
					"(%s) (4) (%s)***An event occurred during rtp operation! bRTP_Play_Break = true***\n",
					(index == 0) ? "Bot_VIB" : "Top_VIB",
					dw791x->fwName));
				ret = -1;
			}
			if (fifo_level >
			    (fifo_size - fifo_size / 3)) // wait interrupt delay
			{
		...
		// step 6: ------------------ wave data transfer done!
			...
			if (repeat > 1) {
				DbgOut((DBL_ERROR,
					"(%s) (%s)The rtp operation size: %d, repeat: %d\n",
					(index == 0) ? "Bot_VIB" : "Top_VIB",
					dw791x->fwName, play_size, repeat));
				curcnt = 0;
				repeat--;
			} else {
				run = 1;
				DbgOut((DBL_ERROR,
					"(%s) (%s)The rtp operation is finished. size: %d, repeat: %d\n",
					(index == 0) ? "Bot_VIB" : "Top_VIB",
					dw791x->fwName, curcnt, repeat));
				dw7914_mode_set(client, D14_FIFO_FLUSH, BITSET);
				break;
			}
		}
...
static int request_transfer_mem_wave(struct i2c_client *client, u32 page,
				     u32 point, u8 *fw_data, u32 size)
{
	int ret = 0;
	u32 i, loop, tail, wave_size, start_addr, set_addr, trans_size,
		fw_point;
	struct dw791x_priv *dw791x = dw791x_devs[getActuatorIndex(client)];

	if (!dw791x->i2c_status) {
		DbgOut((DBL_INFO,
			"request_transfer_mem_wave vibrator %d i2c fail \n",
			getActuatorIndex(client)));
		return -1;
	}
	...
	if (point > 0) {
		...
		dw791x_seq_write(client, dw791x->mem_input, set_addr,
				 RAM_ADDR16, (u8 *)fw_data + fw_point, 5);

		set_addr = (u32)(fw_data[fw_point + 0]) << 8 |
			   fw_data[fw_point + 1];
		wave_size = (u32)(0xF & fw_data[fw_point + 2]) << 8 |
			    fw_data[fw_point + 3];
	} else {
		set_addr = 0;
		wave_size = BIN_BASE;
	...
	for (i = 0; i < loop; i++) {
		...
		dw791x_seq_write(client, dw791x->mem_input, start_addr,
				 RAM_ADDR16, (u8 *)fw_data + fw_point,
				 trans_size);
	}
	...
	if (tail > 0) {
		...
		fw_point = start_addr + BIN_BASE * page;
		dw791x_seq_write(client, dw791x->mem_input, start_addr,
				 RAM_ADDR16, (u8 *)fw_data + fw_point, tail);
	}
...
static int dw791x_device_init(struct i2c_client *client)
{
	...
	if (dw791x != NULL) {
		...
		dw791x_byte_write(
			client, 0x02,
			0xcf); //INTZ_EN, FIFO_FULL, FIFO_EMPTY, UVLO, TSD, OCP, SCP
		//dw791x_byte_write(client, 0x02, 0x0f);//INTZ_EN, UVLO, TSD, OCP, SCP
		//dw791x_byte_write(client, DW7914_PWM, gPWM);//48Khz
		DbgOut((DBL_INFO,
			"Bus=%d, Addr=0x%0x, DWA %s driver version %x\n", bus,
			addr, dw791x->dev_name, dw791x->version));
		dw791x_bst_option(dw791x->i2c, 0, 0, 0); //default
		val = (getActuatorIndex(client) == 0) ?
			      DEFAULT_BOT_VD_CLAMP :
			      DEFAULT_TOP_VD_CLAMP; //set default vd_clamp
		//dw791x->vd_clamp=val;
		dw791x_vd_clamp_set(dw791x->i2c, val);
		//others
		dw791x->bRTP_Play_Break = false;
...
IMMVIBESPIAPI VibeStatus ImmVibeSPI_ForceOut_AmpDisable(VibeUInt8 nActuatorIndex)
{
	...
	if (gVibDebugLog & 0x0004) //enable immersion message
		DbgOut((DBL_INFO,
			"ImmVibeSPI_ForceOut_AmpDisable ActuatorIndex:%d.\n",
			nActuatorIndex));
	if (dw791x->buffering) {
		dw791x->buffering = -1;
...
IMMVIBESPIAPI VibeStatus ImmVibeSPI_ForceOut_AmpEnable(VibeUInt8 nActuatorIndex)
{
	...
	* */
	...
	if (gVibDebugLog & 0x0004) //enable immersion message
		DbgOut((DBL_INFO,
			"ImmVibeSPI_ForceOut_AmpEnable ActuatorIndex:%d.\n",
			nActuatorIndex));
	dw791x->buffering = 1;
	dw791x->position = 1;
	/* read any pending error codes */
	...
	if (result != 2) {
		DbgOut((DBL_ERROR,
			"dw7914: i2c read status failure. result=%d\n",
			result));
	}
	if (status & 0x0F) {
	...
	if (mode != DW7914_MODE_RTP) {
		DbgOut((DBL_ERROR,
			"%s: ActuatorIndex:%d, DW7914_MODE_RTP|DW7914_MODE_FIFO_FLUSH\n",
			__func__, nActuatorIndex));
		dw791x_byte_write(dw791x->i2c, DW7914_MODE,
				  DW7914_MODE_RTP | DW7914_MODE_FIFO_FLUSH);
		dw791x_PWM_set(dw791x->i2c, gPWM);
	}
	//check mode ---
	dw791x->bRTP_Play_Break = true;
...
IMMVIBESPIAPI VibeStatus ImmVibeSPI_ForceOut_Initialize(void)
{
	...
	if (!dma_buf_resampling) {
		dma_buf_resampling =
			kmalloc(1024 * 8 + 1,
				GFP_DMA | GFP_KERNEL); //1028*4+1, 12k upto 48k
		if (dma_buf_resampling) {
			memset(dma_buf_resampling, 0, 1024 * 8 + 1);
		} else {
			DbgOut((DBL_ERROR,
				"The buffer dma_buf_resampling kmalloc failed.\n"));
		}
	}
	if (!dma_buf_ivt) {
		...
		if (dma_buf_ivt) {
			...
			DbgOut((DBL_ERROR,
				"The buffer dma_buf_ivt kmalloc failed.\n"));
		}
	}
	//end
	...
	if (retVal) {
		DbgOut((DBL_ERROR,
			"ImmVibeSPI_ForceOut_Initialize: Cannot add driver.\n"));
	}
	...
	VibeUInt16 nBufferSizeInBytes, VibeInt8 *pForceOutputBuffer)
{
    ...
    */
	...
	if (nBufferSizeInBytes != VIBE_OUTPUT_SAMPLE_SIZE) {
		static VibeInt8 buffer[VIBE_OUTPUT_SAMPLE_SIZE] = {
			0,
		};
...
#endif
	if (VIBE_S_SUCCESS !=
	    I2CWriteWithResendOnError_DW7914(dw791x, nBufferSizeInBytes,
					     pForceOutputBuffer)) {
	...
	// if not currently playing, start playing
	active = DW7914_STATUS1_ACTIVE &
		 dw791x_byte_read(dw791x->i2c, DW7914_STATUS1);
	if (!active) {
					    ...
					    VibeInt8 *pForceOutputBuffer)
{
	...
	if (!pktsize) {
		...
		if (resample > 1) {
			for (i = 0, j = 0; i < nBufferSizeInBytes * resample;
			     i++) {
				dma_buf_resampling[i + 1] = dma_buf_ivt[j];
				if (i % resample == 3) {
					if (gVibDebugLog &
					    0x0008) //enable nForceData
					{
						DbgOut((DBL_WARNING,
							"ivt[%04d]=0x%02x resample[%04d-%04d]=0x%02x 0x%02x 0x%02x 0x%02x\n",
							j, dma_buf_ivt[j],
							j * 4, i,
							dma_buf_resampling[j * 4 +
									   1],
							dma_buf_resampling[j * 4 +
									   2],
							dma_buf_resampling[j * 4 +
			...
			if (gVibDebugLog & 0x0004) //enable immersion message
			{
				DbgOut((DBL_ERROR, "%s: read vd_clamp: %d\n",
					__func__,
					dw791x_byte_read(dw791x->i2c,
							 DW7914_VD_CLAMP) *
						40));
			}
			i2c_send_buf(dw791x->i2c, dma_buf_resampling,
				     nBufferSizeInBytes * resample -
					     nDataIndex + 1);
		} else {
			if (gVibDebugLog & 0x0008) //enable nForceData
			{
			...
			/* Send remaining bytes */
			/* The return value of this function is unreliable so ignoring it */
			i2c_send_buf(dw791x->i2c, dma_buf_ivt,
				     nBufferSizeInBytes - nDataIndex + 1);
		}
	}
#if 0
						   ...
						   char *szDevName, int nSize)
{
	...
	if (dw791x == NULL) {
		...
		if (!dw791x->i2c_status) {
			DbgOut((DBL_INFO,
				"%s: i2c fail, assign default dev name \n",
				__func__));
			sprintf(szRevision, DEVICE_NAME "-DW791x-%02x",
				0x7914F002);
		} else
			sprintf(szRevision, DEVICE_NAME "-DW791x-%02x",
				dw791x->chipid);
...
IMMVIBESPIAPI int ImmVibeSPI_ForceOut_BufferFull(void)
{
	...
	if (dw791x_devs[0]) {
		...
		if (dw791x_devs[0]->i2c) {
			if (dw791x_devs[0]->i2c_status) {
				...
				buf[0] = dw791x_byte_read(dw791x_devs[0]->i2c,
							  0x4D);
				//DbgOut((DBL_INFO, "%s: read bus:0 buffer[1]\n", __func__));
				buf[1] = dw791x_byte_read(dw791x_devs[0]->i2c,
							  0x4E);
				status1 = ((u32)buf[0] << 8) | buf[1];
			} else
				DbgOut((DBL_INFO,
					"%s: read bus:0 buffer failed, status1=0\n",
					__func__));
		}
	}
	...
	if (dw791x_devs[1]) {
		...
		if (dw791x_devs[1]->i2c) {
			if (dw791x_devs[1]->i2c_status) {
				...
				buf[0] = dw791x_byte_read(dw791x_devs[1]->i2c,
							  0x4D);
				//DbgOut((DBL_INFO, "%s: read bus:1 buffer[1]\n", __func__));
				buf[1] = dw791x_byte_read(dw791x_devs[1]->i2c,
							  0x4E);
				status2 = ((u32)buf[0] << 8) | buf[1];
			} else
				DbgOut((DBL_INFO,
					"%s: read bus:1 buffer failed, status2=0.\n",
					__func__));
		}
	}
	...
	if (status1 >= DW791X_FIFO_LEVEL)
		DbgOut((DBL_INFO, "%s: status1 >= DW791X_FIFO_LEVEL\n",
			__func__));
	if (status2 >= DW791X_FIFO_LEVEL)
		DbgOut((DBL_INFO, "%s: status2 >= DW791X_FIFO_LEVEL\n",
			__func__));
	return status1 > status2 ? (status1 >= DW791X_FIFO_LEVEL) :
				   (status2 >= DW791X_FIFO_LEVEL);
}
#endif
